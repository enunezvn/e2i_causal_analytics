...
def _hermetic_memory_layer(monkeypatch):
    ...
    Before #883 the resource_optimizer episodic write and procedural pattern
    write ALWAYS failed against the live enums (22P02 swallowed into
    ``logger.warning`` -> None) and the pattern READ died on a nonexistent
    import (-> []), so this suite was de-facto hermetic even where tests call
    store/contribute paths unmocked. Now that the writes actually land, an
    unmocked run from a creds-configured dev box inserts REAL rows into the
    live DB mid-unit-run (observed: episodic + procedural strays, plus
    order-dependent failures when a stray pattern row satisfied a
    graceful-degradation read). Simulate the "memory unavailable" baseline
    instead: the source-module functions raise, the hooks swallow and return
    None/[], matching the suite's historical expectations. Tests exercising a
    specific memory behavior patch these symbols themselves (an inner
    ``with patch`` wins over this fixture).
    """
