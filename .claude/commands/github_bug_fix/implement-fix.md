...
Before proceeding, validate the GitHub issue ID.

```bash
# Required GitHub issue ID
if [ -z "$ARGUMENTS" ]; then
  echo "❌ Error: GitHub issue ID is required"
  echo ""
  echo "Usage: /github_bug_fix:implement-fix <github-issue-id>"
  echo ""
  echo "Examples:"
  echo "  /github_bug_fix:implement-fix 123"
  echo "  /github_bug_fix:implement-fix 456"
  exit 1
fi

# Validate it's a number
if ! [[ "$ARGUMENTS" =~ ^[0-9]+$ ]]; then
  echo "❌ Error: Invalid issue ID. Must be a positive integer."
  echo "Received: $ARGUMENTS"
  exit 1
fi

ISSUE_ID="$ARGUMENTS"
echo "✓ GitHub Issue: #$ISSUE_ID"
```
