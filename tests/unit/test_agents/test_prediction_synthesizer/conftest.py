...
def _hermetic_memory_layer(monkeypatch):
    ...
    Before #883 the prediction_synthesizer episodic write ALWAYS failed
    against the live memory_outcome_type enum (22P02 swallowed into
    ``logger.warning`` -> None), so this suite was de-facto hermetic even
    where tests call store/contribute paths unmocked (e.g.
    test_store_prediction_graceful_failure relied on that silent failure).
    Now that the write actually lands, an unmocked run from a
    creds-configured dev box inserts REAL rows into the live DB
    mid-unit-run. Simulate the "memory unavailable" baseline instead: the
    source-module functions raise, the hooks swallow and return None/[],
    matching the suite's historical expectations. Tests exercising a
