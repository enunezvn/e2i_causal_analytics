...
def test_compile_set_preserves_legacy_demo_roles():
    """#519 resolution (b): every legacy (pre-Option-C) feature is authored in
    the compile-set CANDIDATE POOL (``build_compile_set()``) with a cohort-only
    variant carrying its labeled causal_role.

    Re-scoped 2026-05-26 (#519): the original form asserted all 21 legacy demos
    persist in the SHIPPED artifact. But #239 promoted a MIPROv2 compile whose
    80/20 split holds out a 48-example val set that is NOT persisted as demos —
    so whichever legacy demos land in the held-out 48 read as "dropped" even
    when the recompile is correct (this caused the pre-existing failure #519
    quarantined). The binding invariant is the authored training signal (the
    candidate pool), not which examples MIPROv2 keeps in its train split:
      * the sole production caller (`adaptive_validity_check.py:892-893`) emits
        cohort-only `dataset_context`, so persistence of any *specific* demo is
        not what production depends on — the authored teaching signal is; and
      * AC3 instrument precision = 1.000 on the current 192-demo artifact
        (#517 re-measure) shows the split is sufficient for the gated metric.
    Artifact-level signal is still guarded by
    ``test_persisted_artifact_has_treatment_outcome_explicit_instruments`` (>=2
    (T,Y) instrument demos persist) and the #517 source-consistency guard.
    """
    from src.data.causal_role_classifier import build_compile_set

    examples = build_compile_set()

    # Find the LEGACY (cohort-only) variant per feature_name. Exclude Phase-4
    # paired-fixture variants (which carry `treatment=` in dataset_context).
    legacy_by_feature: dict[str, list] = {}
    for ex in examples:
        if ex.feature_name not in LEGACY_DEMO_EXPECTED_ROLES:
            continue
        if "treatment=" in (ex.dataset_context or ""):
            continue
        legacy_by_feature.setdefault(ex.feature_name, []).append(ex)

    missing_legacy = set(LEGACY_DEMO_EXPECTED_ROLES) - set(legacy_by_feature)
    assert not missing_legacy, (
        f"compile-set candidate pool is missing legacy cohort-only exemplars: "
        f"{sorted(missing_legacy)}. build_compile_set() must author every legacy "
        f"feature with a cohort-only variant (the sole production caller at "
        f"adaptive_validity_check.py:892-893 emits cohort-only contexts)."
    )

    role_mismatches: list[tuple[str, str, str]] = []
    for feature_name, expected_role in LEGACY_DEMO_EXPECTED_ROLES.items():
        for ex in legacy_by_feature[feature_name]:
            if ex.causal_role != expected_role:
                role_mismatches.append((feature_name, expected_role, ex.causal_role or "?"))

    assert not role_mismatches, (
        f"compile-set legacy exemplars carry the wrong causal_role "
        f"(feature, expected, actual): {role_mismatches}. build_compile_set() "
        f"must author each legacy feature with its labeled role."
    )


def test_compile_set_emits_role_conditional_on_treatment_outcome():
    """#519 resolution (b): all 12 paired-fixture quadruples
    `(feature_name, treatment, outcome, role)` are authored in the compile-set
    candidate pool (``build_compile_set()``) — the same feature with the same
    `derivation_pseudocode` classifies into a different causal role under
    different (T, Y) variants. This is the instrument-role schema-gap-closure
    training signal; asserting all 12 quadruples (not 6 feature_names) guards
    that no paired variant is dropped from the authored pool.

    Re-scoped 2026-05-26 (#519): the original form required all 12 to persist in
    the SHIPPED artifact, but MIPROv2's 80/20 split (per #239) may hold some out
    of the persisted 192 demos (see ``test_compile_set_preserves_legacy_demo_roles``
    for the full rationale). The binding invariant is that the authored training
    signal exists in the candidate pool; the persisted split is a downstream
    optimizer partition. (>=2 (T,Y)-explicit instrument demos DO persist in the
    artifact — guarded separately by
    ``test_persisted_artifact_has_treatment_outcome_explicit_instruments``.)
    """
    from src.data.causal_role_classifier import build_compile_set
    ...
    for feature_name, treatment, outcome, expected_role in EXPECTED_TREATMENT_OUTCOME_QUADRUPLES:
        ...
        match = next(
            (
                ex
                for ex in examples
                if ex.feature_name == feature_name
                and marker in (ex.dataset_context or "")
                and ex.causal_role == expected_role
            ),
            None,
        )
        if match is None:
    ...
    assert not missing_quadruples, (
        f"#519: the following (feature, T, Y, role) quadruples are NOT authored "
        f"in build_compile_set(): {missing_quadruples}. build_compile_set() must "
        f"define all 12 paired-fixture demos so the classifier's training signal "
        f"encodes that the role depends on (T, Y), not on feature_name alone."
    )
