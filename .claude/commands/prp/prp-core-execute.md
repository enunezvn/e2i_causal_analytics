...
Before proceeding, validate the PRP file path.

```bash
# Required PRP file path
if [ -z "$ARGUMENTS" ]; then
  echo "❌ Error: PRP file path is required"
  echo ""
  echo "Usage: /prp:prp-core-execute <path-to-prp>"
  echo ""
  echo "Available PRPs:"
  find .claude/PRPs/features -name "*.md" -type f 2>/dev/null | head -10
  exit 1
fi
...
if [[ "$PRP_FILE" != *.md ]]; then
  echo "⚠️  Warning: PRP file should be a .md file"
  echo "Received: $PRP_FILE"
  echo ""
  echo "Continuing anyway..."
fi

echo "✓ Using PRP: $PRP_FILE"
```
