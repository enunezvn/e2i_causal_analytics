...
def _run_feast(*args: str, cwd: Path = PROJECT_ROOT) -> subprocess.CompletedProcess[str]:
    """Run a ``feast`` subcommand and return the completed process."""
    cmd = ["feast", "--chdir", str(FEATURE_REPO.relative_to(cwd))] + list(args)
...
def test_registry_db_not_tracked_in_git() -> None:
    ...
    regenerated by ``feast apply`` and committing it would create churn and
    desync from the Python source of truth.
    """
