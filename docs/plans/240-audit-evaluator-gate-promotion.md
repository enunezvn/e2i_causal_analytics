...
### Aggregate result (committed in `artifacts/dspy/ac3_verdict_n200.json`)

| cohort | bootstrap.gated.precision_instrument | miprov2.gated.precision_instrument | ≥? |
|--------|--------------------------------------|------------------------------------|----|
| OVERALL | 1.000 (TP=3, FP=0) | 1.000 (TP=6, FP=0) | ✓ |
| CSU_remibrutinib | n/a (no gated instruments) | n/a | n/a (tie) |
| PNH_fabhalta | 1.000 (TP=1, FP=0) | 1.000 (TP=2, FP=0) | ✓ |
| BC_kisqali | 1.000 (TP=2, FP=0) | 1.000 (TP=4, FP=0) | ✓ |

### What is and is not in the committed artifact

- **In the artifact**: the AC3 verdict JSON (38 lines, schema_version=1) with per-cohort gated `precision_instrument` for both optimizers + the rule decision (`miprov2_wins=true`). This is the basis for the optimizer-promotion decision in #477; it is **insufficient** for evaluating gate-promotion rules in this proposal because it aggregates over the gate decision.
...
**Conclusion for this proposal:** the n=240 / golden-n=91 + `--enable-evaluator` run *did execute the evaluator* end-to-end and produced per-entry audit data at run time, but the per-entry payload was not persisted to a committed artifact. **A Stage-0 (data collection) step is therefore the first pre-requisite of any gate promotion** — see §4.

### What the aggregate signal *does* tell us

1. **The evaluator is consequential as a filter.** The gated subset's TP counts (3/6 in bootstrap/miprov2 vs ungated TP=18/19 reported in earlier n=50 runs per PR #469 memory) imply the gate excludes ~17/24 of instrument predictions — a high-impact filter. This means promotion is not academic: a gate built on `evaluator_satisfied=False` would touch a meaningful slice of verdicts.
2. **The gate appears precision-preserving at the aggregate.** Both optimizers achieved `precision_instrument=1.000` on the gated subset (no FPs survived the filter). This is consistent with the evaluator's design intent but does not establish that severity-modulation rules built on the same signal would have the same property.
3. **The CSU cohort produced no gated instruments at all** — `n/a` for both optimizers. Any severity-modulation rule must explicitly handle the empty-gated-cohort case (fail-open).
...
- Add a new column to `adaptive_validity_verdicts` (migration 041): `would_promote_severity` (text, nullable) — populated by a pure function `evaluate_promotion_rule(worker_verdict, evaluator_audit) -> Optional[str]` whose contract is: `None` when no rule fires, else the proposed escalated severity. **The voter does not read this column.** It is written for analytics only.
- New file `src/data/evaluator_promotion_rules.py` containing the rule function + a `PROMOTION_RULES` registry (initially empty or single dry-run rule).
- Extend `_ensemble_to_legacy_dict` to compute and surface `would_promote_severity` into the sidecar row.
...
- AC1.2 — A new test asserts that for ≥99% of runs where the rule fires, `verdict.severity` is unchanged vs the equivalent run with the rule disabled (i.e., shadow truly shadows).
...
- When `1`, after the voter computes its candidate severity, the voter calls `evaluate_promotion_rule(...)` exactly once. If the rule fires AND the candidate severity equals the rule's precondition, the voter substitutes the rule's proposed severity AND appends to `evidence` a structured tag `"evaluator_gate:R1:moderate→high"`. The `decided_by` field gains a new value `"evaluator_gate"` ONLY when the gate actually flipped a decision (not when the voter independently reached the same severity).
...
- AC3.1 — At Stage 3 enabled for 30 days against a non-production validation cohort, `precision_instrument` on the gated subset of the golden set is **strictly ≥** the un-gated baseline measured in #477 (`overall.gated.precision_instrument` was 1.000 at n=240 — must remain 1.000 OR improve to a measurable cohort that was `n/a` before).
...
**Status in this proposal:** **deferred**. Stage 4 acceptance criteria are intentionally not specified — they require Stage 3 operational data we do not have. This proposal commits only to *not closing the door* on Stage 4: the Stage 1–3 instrumentation (shadow column, kill-switch, `gate_rule_fired` field) is designed to be a strict subset of what Stage 4 would need.
...
- **Stage-1 action:** record `would_promote_severity = worker_verdict.severity` (no-op for severity), but set a separate `would_flag_for_review = True` column.
...
- **Stage-1 action:** record in a separate column `rationale_incomplete_flag = True`.
...
| 1 | AC1.2 | Shadow-mode invariant test: severity unchanged in ≥99% of rule-firing cases |
