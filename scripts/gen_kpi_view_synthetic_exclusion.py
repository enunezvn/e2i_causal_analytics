...
def wrap_table(sql, table):
    pat = re.compile(r"\b(FROM|JOIN)\s+(?:public\.)?" + re.escape(table)
                     + r"\b(\s+(?:AS\s+)?([A-Za-z_][A-Za-z0-9_]*))?", re.IGNORECASE)
...
def main():
    ...
    for t in NEW_TAG_TABLES:
        parts.append(f"ALTER TABLE {t} ADD COLUMN IF NOT EXISTS is_synthetic BOOLEAN NOT NULL DEFAULT false;")
    parts.append("")
    for v in VIEWS:
        body = psql(f"SELECT pg_get_viewdef('public.{v}'::regclass, true);").strip().rstrip(";").strip()
        for t in TAGGABLE:
