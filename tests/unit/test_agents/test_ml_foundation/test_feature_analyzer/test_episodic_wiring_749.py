"""#749: feature_analyzer must record its run to EPISODIC memory. Its store_feature_analysis hook was defined but
...
@pytest.mark.unit
def test_update_episodic_memory_invokes_store_feature_analysis_with_uuid_session():
    ...
    with patch(
        ...
        hook.store_feature_analysis = AsyncMock(return_value="mem-1")
        asyncio.run(agent._update_episodic_memory(_DATA))

    hook.store_feature_analysis.assert_awaited_once()
    kwargs = hook.store_feature_analysis.await_args.kwargs
