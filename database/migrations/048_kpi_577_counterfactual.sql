...
INSERT INTO public.kpi_query_registry (query_id, sql, max_params, note) VALUES
    ('causal_metrics_counterfactual', $kpi$SELECT AVG(counterfactual_outcome)::float AS mean_counterfactual, AVG(prediction_value)::float AS mean_factual, AVG(treatment_effect_estimate)::float AS mean_effect, COUNT(*)::int AS n FROM ml_predictions WHERE counterfactual_outcome IS NOT NULL AND treatment_effect_estimate IS NOT NULL AND ($1 = '' OR prediction_type::text = $1)$kpi$, 1, $note$CM-004 counterfactual: counterfactual outcome LEVEL E[Y(a')] = mean(counterfactual_outcome), where counterfactual_outcome = GREATEST(0, prediction_value − treatment_effect_estimate) (a real do-contrast: per-row factual − counterfactual = the treatment effect). $1 = prediction_type filter ('' = all). mean_factual + mean_effect (the contrast) returned for context. Distinct from CM-001 ATE (the contrast). NULL mean_counterfactual (fail-loud) when no coherent rows match.$note$)
