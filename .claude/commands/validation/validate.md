...
Before proceeding, validate the ngrok URL.
...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo ""
  echo "Usage: /validation:validate <ngrok-url>"
  echo ""
  echo "Example:"
  echo "  /validation:validate https://trinity-nonadverbial-enharmonically.ngrok-free.dev"
  echo ""
  echo "Prerequisites:"
  echo "  - ngrok running and exposing port 3000"
...
if [[ ! "$ARGUMENTS" =~ ^https?:// ]]; then
  ...
  echo ""
  echo "Example: https://trinity-nonadverbial-enharmonically.ngrok-free.dev"
  exit 1
fi
...
cp "${PROJECT_ROOT}/.claude/commands/prime.md" .claude/commands/
cp "${PROJECT_ROOT}/.claude/commands/plan-feature.md" .claude/commands/
cp "${PROJECT_ROOT}/.claude/commands/execute.md" .claude/commands/
...
gh issue comment ${COMMAND_ISSUE_NUMBER} \
  --body "@remote-agent /load-commands .claude/commands
...
# Check logs
docker-compose logs app --tail 150 | grep -E "plan-feature|Plan|\.claude/PRPs/features/active" | tail -30
```
...
if [ -d ".claude/PRPs/features/active" ]; then
  echo "✅ .claude/PRPs/features/active directory exists"
  ls -la .claude/PRPs/features/active/

  # Store plan file name
  PLAN_FILE=$(ls .claude/PRPs/features/active/ | head -1)
  echo "Plan file created: ${PLAN_FILE}"
else
  echo "❌ .claude/PRPs/features/active directory not found"
fi
...
**Expected:**
- ✅ `.claude/PRPs/features/active/<feature-name>.md` file created
...
# Bot should report: "Plan file: .claude/PRPs/features/active/<filename>.md"
...
if [ -z "$PLAN_FILE_PATH" ]; then
  ...
  PLAN_FILE_PATH=$(ls .claude/PRPs/features/active/*.md | head -1)
  git checkout main
fi
...
- ✅ Plan file path extracted (e.g., `.claude/PRPs/features/active/add-contributing-section.md`)
...
if [ -n "$COMMAND_PR_NUMBER" ]; then
  ...
  if echo "$PR_BODY" | grep -q ".claude/PRPs/features/active"; then
...
echo "  1. /load-commands .claude/commands"
echo "  2. /command-invoke prime"
echo "  3. /command-invoke plan-feature <feature description>"
echo "  4. /command-invoke execute .claude/PRPs/features/active/<plan-file>"
echo ""
echo "Outputs:"
echo "  - Plan File: .claude/PRPs/features/active/${PLAN_FILE}"
