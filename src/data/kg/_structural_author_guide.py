"""Sections 0–6 of ``docs/layer4/structural_attestation_authoring.md``, VERBATIM.

Lane B of the real-data causal estimation program (2026-09-22): the structural
author's DSPy instructions are the guide's sections 0–6. The docs tree is
excluded from the API image (``.dockerignore:76``), so the text is embedded
here; ``tests/unit/test_data/test_kg/test_structural_author.py::
test_guide_sections_are_verbatim`` pins it byte-for-byte to the guide, and
``GUIDE_HASH`` is stamped on every authored record.
"""
...
#: Relative path of the guide whose sections 0–6 are embedded below.
GUIDE_PATH = "docs/layer4/structural_attestation_authoring.md"

#: The heading the embedded text starts at (everything from here to EOF).
GUIDE_FIRST_HEADING = "## 0. The one rule that matters most"

GUIDE_SECTIONS_0_TO_6 = """## 0. The one rule that matters most
...
#: sha256 of the embedded guide text — stamped on every authored record.
GUIDE_HASH = hashlib.sha256(GUIDE_SECTIONS_0_TO_6.encode("utf-8")).hexdigest()
