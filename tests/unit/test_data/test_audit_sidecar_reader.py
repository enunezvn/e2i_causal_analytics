...
def test_sidecar_payload_includes_schema_version_v1(tmp_path, monkeypatch):
    ...
    state = {
        ...
        # Layer-4 Phase 1 (schema 1.6): run-level FDR firing-driver summary.
        "leakage_fdr": {
            "active": True,
            "enabled": True,
            "q": 0.1,
    ...
    assert payload.get("schema_version") == "1.8", (
    ...
    assert payload.get("leakage_fdr") == {
        ...
        "confident_features": ["f"],
        "reason": "active",
    }, f"sidecar must persist leakage_fdr; got {payload.get('leakage_fdr')!r}"
...
def test_reader_warns_on_unknown_schema_version_major(tmp_path, caplog):
    ...
    # Phase 2.6 citation channel (#1608): reader's current version is "1.8"
    # (still MAJOR=1).
    matches = [
        rec
        for rec in caplog.records
        if "schema_version" in rec.message and "2.0" in rec.message and "1.8" in rec.message
    ]
    assert matches, (
