...
def remibrutinib_patient_df():
    ...
    return pd.DataFrame({
        ...
        "antihistamine_refractory": [True, False, True, True, True, False],  # P002 and P006 not refractory
...
def fabhalta_patient_df():
    ...
    return pd.DataFrame({
        ...
        "diagnosis_date": ["2023-01-15"] * 5,
        "complement_inhibitor_status": ["naive", "experienced", "naive", "experienced", "naive"],
...
def kisqali_patient_df():
    ...
    return pd.DataFrame({
        ...
        "diagnosis_date": ["2023-01-15"] * 5,
        "hr_status": ["positive", "positive", "negative", "positive", "positive"],  # P003 is HR-
        "her2_status": ["negative", "positive", "negative", "negative", "negative"],  # P002 is HER2+
...
def empty_patient_df():
    ...
    return pd.DataFrame(columns=[
        "patient_journey_id", "age_at_diagnosis", "gender",
...
def sample_criterion():
    ...
    from src.agents.cohort_constructor import Criterion, Operator, CriterionType
...
def mock_opik_tracer(mocker):
    ...
    mock.trace_cohort_construction.return_value.__enter__ = mocker.MagicMock(return_value=mock_context)
