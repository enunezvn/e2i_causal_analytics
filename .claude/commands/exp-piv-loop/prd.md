...
Before proceeding, validate and set the output filename.

```bash
# Optional argument with default
if [ -z "$ARGUMENTS" ]; then
  OUTPUT_FILE="PRD.md"
  echo "ℹ️  No filename provided, using default: PRD.md"
else
  OUTPUT_FILE="$ARGUMENTS"

  # Check if it has .md extension, add if missing
  if [[ "$OUTPUT_FILE" != *.md ]]; then
    OUTPUT_FILE="${OUTPUT_FILE}.md"
    echo "ℹ️  Added .md extension: $OUTPUT_FILE"
  fi
...
if [ -f "$OUTPUT_FILE" ]; then
  echo "⚠️  Warning: File already exists: $OUTPUT_FILE"
  echo "Will be overwritten with new PRD"
fi
```
