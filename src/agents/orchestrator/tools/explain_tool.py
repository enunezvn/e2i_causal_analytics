        ...
        for pattern in patterns:
            if re.search(pattern, query_lower):
                sub_intent = ExplainSubIntent(
                    sub_intent_key.replace("why_patient", "patient_prediction")
                )
...
class ExplanationEntities(BaseModel):
    ...
    patient_ids: List[str] = Field(
        default_factory=list, description="Multiple patient IDs for batch/compare"
    )
...
class ExplainAPITool:
    ...
    def __init__(self, api_base_url: str = "http://localhost:8000/api/v1", timeout: float = 10.0):
    ...
    async def get_explanation_history(self, patient_id: str, limit: int = 10) -> Dict[str, Any]:
        """Get explanation history for a patient."""
        try:
            response = await self.client.get(
                f"{self.api_base_url}/explain/history/{patient_id}", params={"limit": limit}
            )
            response.raise_for_status()
            return {"success": True, "data": response.json()}
    ...
    async def batch_explain(
        self, patient_ids: List[str], model_type: str = "propensity"
    ) -> Dict[str, Any]:
        """Get explanations for multiple patients."""
        try:
            requests = [
                {"patient_id": pid, "model_type": model_type, "top_k": 5, "store_for_audit": True}
...
class ExplanationResponseFormatter:
    ...
    @staticmethod
    def format_comparison_response(explanations: List[Dict[str, Any]]) -> Dict[str, Any]:
...
class ExplainIntentHandler:
    ...
    async def _handle_patient_explanation(self, entities: ExplanationEntities) -> Dict[str, Any]:
    ...
    async def _handle_history(self, entities: ExplanationEntities) -> Dict[str, Any]:
        ...
        result = await self.api_tool.get_explanation_history(
            patient_id=entities.patient_id, limit=10
        )
        ...
        if result["success"]:
            ...
            return {
                "type": "error",
                "message": f"Unable to retrieve history: {result.get('error')}",
            }

    async def _handle_comparison(self, entities: ExplanationEntities) -> Dict[str, Any]:
    ...
    async def _handle_feature_importance(
        self, entities: ExplanationEntities, query: str
    ) -> Dict[str, Any]:
