...
def build_hcp_expectations(dataset: "PandasDataset") -> "PandasDataset":
    ...
    dataset.expect_column_mean_to_be_between(
        "years_experience", min_value=10, max_value=25
    )
    dataset.expect_column_mean_to_be_between(
        "total_patient_volume", min_value=100, max_value=400
    )
...
def build_patient_journey_expectations(
    ...
    dataset.expect_column_values_to_be_between(
        "disease_severity", min_value=0, max_value=10
    )
    dataset.expect_column_values_to_be_between(
        "engagement_score", min_value=0, max_value=10
    )
    dataset.expect_column_values_to_be_in_set(
    ...
    dataset.expect_column_mean_to_be_between(
        "disease_severity", min_value=3.0, max_value=7.0
    )
    dataset.expect_column_stdev_to_be_between(
        "disease_severity", min_value=1.0, max_value=3.0
    )

    # Academic HCP proportion (~30%)
    dataset.expect_column_mean_to_be_between(
        "academic_hcp", min_value=0.20, max_value=0.40
    )
...
def build_treatment_event_expectations(dataset: "PandasDataset") -> "PandasDataset":
    ...
    # Value ranges
    dataset.expect_column_values_to_be_between(
        "adherence_score", min_value=0, max_value=1
    )
    dataset.expect_column_values_to_be_between(
        "efficacy_score", min_value=0, max_value=1
    )
    dataset.expect_column_values_to_be_between(
        "duration_days", min_value=1, max_value=365
    )
    ...
    # Statistical expectations
    dataset.expect_column_mean_to_be_between(
        "adherence_score", min_value=0.60, max_value=0.90
    )
    dataset.expect_column_mean_to_be_between(
        "efficacy_score", min_value=0.50, max_value=0.80
    )
...
def build_ml_prediction_expectations(dataset: "PandasDataset") -> "PandasDataset":
    ...
    dataset.expect_column_values_to_be_between(
        "prediction_value", min_value=0, max_value=1
    )
    dataset.expect_column_values_to_be_between(
        "confidence_score", min_value=0, max_value=1
    )
    dataset.expect_column_values_to_be_between(
...
def build_trigger_expectations(dataset: "PandasDataset") -> "PandasDataset":
    ...
    # Value ranges
    dataset.expect_column_values_to_be_between(
        "priority", min_value=1, max_value=5
    )
    dataset.expect_column_values_to_be_between(
        "confidence_score", min_value=0, max_value=1
    )
    dataset.expect_column_values_to_be_between(
        "lead_time_days", min_value=0, max_value=365
    )
    ...
    dataset.expect_column_values_to_be_in_set(
        "delivery_channel",
        ["email", "call", "in_person", "portal"]
    )

    # Priority distribution (should have mix of priorities)
    dataset.expect_column_mean_to_be_between(
        "priority", min_value=2.0, max_value=4.0
    )
...
def get_checkpoint_summary(results: Dict[str, ValidationResult]) -> str:
    ...
    lines.append(f"Overall: {overall_status} ({total_passed}/{total_expectations} expectations, {overall_rate:.1f}%)")
    lines.append("=" * 60)
