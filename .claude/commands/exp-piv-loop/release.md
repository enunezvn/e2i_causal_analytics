...
Before proceeding, validate the version argument.
...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo "  /release 1.2.0"
  echo "  /release v1.2.0"
  echo "  /release 1.2.0-alpha.1"
  echo ""
  echo "To determine version, run /changelog-release first."
  exit 1
fi
...
gh release create "v0.3.0" \
  --title "v0.3.0" \
  --notes-file .claude/PRPs/releases/v0.3.0-notes.md
```
...
gh release create "v0.3.0-alpha.1" \
  ...
  --notes-file .claude/PRPs/releases/v0.3.0-alpha.1-notes.md \
  --prerelease
```
