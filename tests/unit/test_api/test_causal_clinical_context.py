...
async def test_endpoint_forwards_the_treatment_to_the_service():
    ...
    mock_get.assert_called_once_with("Kisqali", "persistent_180d", treatment="copay_support")
...
def test_endpoint_treatment_is_optional_brand_level_view_still_works():
    ...
    assert response.status_code == 200
    mock_get.assert_called_once_with("Kisqali", "persistent_180d", treatment=None)
    body = response.json()
