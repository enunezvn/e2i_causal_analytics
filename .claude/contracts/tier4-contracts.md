...
> **Note — required vs. runtime behavior (verified 2026-07-29, #1344)**: the contract above states the *intended* required shape (`allocation_targets` and `constraints` with `min_items=1`). The **runtime** input model in `src/agents/resource_optimizer/agent.py` is more permissive — it defaults both `allocation_targets` and `constraints` to empty lists (`Field(default_factory=list)`), and likewise defaults `query` and `resource_type`. Consequently a bare "optimize my budget" query with no targets and no constraints will **not** raise a validation error at the model layer, but it has **nothing to solve** (no entities to allocate across, no budget/capacity limits). This is documented current behavior; aligning the code to the `min_items=1` contract is out of scope here.
...
|---------|------|---------|--------|
| 1.1 | 2026-07-29 | Doc-rot note (#1344): documented that Resource Optimizer's runtime input defaults `allocation_targets`/`constraints` to empty lists (contract `min_items=1` is intent, not enforced) — a bare "optimize my budget" query does not error but has nothing to solve | Claude |
