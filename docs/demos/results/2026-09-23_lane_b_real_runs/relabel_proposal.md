...
- authored records flagged ambiguous: 73; review_required: 0

Rule applied: `machine_reviewed` only where the author's fragment reproduces the manifest's edge set exactly AND derives the same role — two independent machine readings that coincide. A disagreement is NOT a verdict against either side; it is the list the owner reads (both rationales below), and stays `machine`.
...
## Agreements (recommended `machine_reviewed`)
