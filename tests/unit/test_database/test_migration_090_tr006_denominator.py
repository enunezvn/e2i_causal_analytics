"""Migration 090 content lock — WS2-TR-006 Override Rate denominator (#1119).

The documented formula is ``count(overridden) / count(delivered)``
(config/kpi_definitions.yaml, docs/data/06-KPI-REFERENCE.md), but migrations
078/089 registered a denominator of ALL non-null acceptance_status rows —
which degenerates to COUNT(*) because the DGP never emits a NULL status
(pending included). Migration 090 re-registers the four override_rate
variants with a delivered-based denominator:
...
MIGRATION = (
    REPO_ROOT / "database" / "migrations" / "090_kpi_tr006_override_delivered_denominator.sql"
)

OVERRIDE_QUERY_IDS = [
    "trigger_performance_override_rate",
    "trigger_performance_override_rate_include_synthetic",
    "trigger_performance_override_rate_region",
    "trigger_performance_override_rate_region_include_synthetic",
]
...
def _registration_lines(content: str) -> dict[str, str]:
    """Map each override_rate query_id to its VALUES line in the migration."""
    lines: dict[str, str] = {}
    for line in content.splitlines():
        for qid in OVERRIDE_QUERY_IDS:
            if f"('{qid}'" in line:
                lines[qid] = line
    return lines
...
def test_all_four_override_variants_reregistered():
    content = MIGRATION.read_text()
    lines = _registration_lines(content)
    missing = [qid for qid in OVERRIDE_QUERY_IDS if qid not in lines]
    assert not missing, f"override_rate variants not re-registered: {missing}"
...
def test_denominator_is_delivered_based():
    ...
    for qid, line in _registration_lines(content).items():
        ...
        assert "acceptance_status IS NOT NULL" not in line, (
            ...
            "COUNT(*)) must not survive migration 090"
        )
...
def test_frontier_anchoring_and_data_through_preserved():
    """090 must keep the 089 frontier-anchoring contract: window anchored at
...
def test_numerator_still_counts_overridden():
    content = MIGRATION.read_text()
    for qid, line in _registration_lines(content).items():
        assert "acceptance_status = 'overridden'" in line, f"{qid}: numerator lost"
