"""#2068: explainer episodic rows must key on the REAL chat session uuid.

``store_explanation`` used to pre-hash the session id with a local
...
async def _stored_session_record(mock_supabase, session_id: str) -> dict:
    ...
    with (
        ...
        memory_id = await ExplanationMemoryHooks().store_explanation(
