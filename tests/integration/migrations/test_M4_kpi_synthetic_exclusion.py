...
def _sql_for(query_id: str) -> str:
    out = subprocess.run(
        [
            ...
            "postgres",
            "-tAc",
            f"SELECT sql FROM kpi_query_registry WHERE query_id='{query_id}';",
        ],
        capture_output=True,
        text=True,
        check=True,
    ...
    try:
        default = subprocess.run(
            [
                ...
                "postgres",
                "-tAc",
                "SELECT * FROM kpi_query('business_impact_trx', '[\"Kisqali\"]'::jsonb);",
            ],
            capture_output=True,
            text=True,
            check=True,
        ...
        subprocess.run(
            [
                ...
                "postgres",
                "-c",
                "DELETE FROM treatment_events WHERE treatment_event_id='SYN_TEST_M4';",
            ],
            check=True,
            capture_output=True,
            text=True,
