...
| **T6 — Robustness** | typos, ambiguity | Typos silently corrected; ambiguous queries produce honest scope-limiting, not hallucination. **Updated 2026-08-03 (measured):** since #1407 shipped, `/chat/stream` delivers a real ask-back for underspecified `kpi_query`/`causal_analysis` turns. **A.5 now clarifies instead of guessing (5.6 s) and resumes to a grounded answer** — T6's ambiguity criterion passes on this surface for the first time. The inverse failure was checked and did not occur: 4.3/5.7 do not clarify in narrative order. **Corrected 2026-08-18:** 5.5 HAS clarified in 4 consecutive AG-UI runs (08-15, 08-16, 08-18×2) — treat a 5.5 clarify as its expected behavior, and grade it on whether the clarify honestly reflects session history (both 08-18 runs asserted a false negative about the session's own tool history — the reproducing defect) |
...
|---|---|---|---|
| **#1407** stateful multi-turn clarification ask-back | 2026-08-02 `23494dd5` | `/chat/stream` only | ✅ **MEASURED 2026-08-03 — clean.** A.5 clarifies (5.6 s) and resumes to a real answer; 4.3/5.7 do **not** clarify in narrative order, so no over-abstention (**2026-08-18 correction:** 5.5 now clarifies consistently — 4 straight AG-UI runs — see T6 row). Full evidence table in the Clarification section |
