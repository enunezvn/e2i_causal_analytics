...
```bash
# Check if issue/error description is provided
if [ -z "$1" ]; then
  echo "❌ Error: Issue, error, or stacktrace is required"
  echo ""
  echo "Usage: /rca <issue|error|stacktrace> [quick]"
  echo ""
  echo "Examples:"
  echo "  /rca \"TypeError: Cannot read property 'map' of undefined\""
  echo "  /rca \"SSH clone URLs not working\" quick"
  echo "  /rca \"Server crashes on startup\""
  exit 1
fi

# Check minimum length
if [ ${#1} -lt 5 ]; then
  echo "❌ Error: Issue description too short (minimum 5 characters)"
  exit 1
fi

# Check for mode flag
if [ -n "$2" ] && [ "$2" != "quick" ]; then
  echo "⚠️  Warning: Unknown mode '$2' - using 'deep' mode"
  echo "Valid modes: 'quick' or leave blank for deep analysis"
  MODE="deep"
else
  MODE="${2:-deep}"
  if [ "$MODE" = "quick" ]; then
    echo "ℹ️  Using quick mode (2-3 Whys, ~5 minute investigation)"
  else
    echo "ℹ️  Using deep mode (full 5 Whys, git history analysis)"
  fi
fi

ISSUE_INPUT="$1"
echo "✓ Issue: $ISSUE_INPUT"
```
...
1. Create directory if needed: `mkdir -p .claude/PRPs/rca`
2. List existing reports: `ls .claude/PRPs/rca/`
3. Find next available number (if `rca-report-1.md` and `rca-report-2.md` exist, use `3`)

**Save to:** `.claude/PRPs/rca/rca-report-{N}.md`
