"""Plan v4 Gate G5 — pre-spec dataset-hash verifier.

Closes G5 codex M1: the spec memo
``docs/specs/g5_coefficient_sensitivity_prespec_20260510.md`` pins
sha256 values for the cohort artifacts referenced by the G5
integration test. This script re-computes the live hashes and
...
Verify (CI mode):
    python scripts/verify_g5_prespec_hashes.py
    # exits non-zero on hash mismatch.
...
def _parse_pinned_hashes(memo_text: str) -> Dict[str, Optional[str]]:
    ...
    Tolerates the dual indentation patterns ``  optum_…:`` (2-space)
    that markdown-rendered YAML produces.
    """
...
def _verify(pinned: Dict[str, Optional[str]]) -> int:
    ...
    for memo_key, (relpath, _) in ARTIFACTS.items():
        ...
        if live != pinned_value:
            failures.append(
                ...
                "g5_*_prespec_<date>.md memo at a fresh date."
            )
            continue
        verified.append(f"{memo_key}: OK (sha256={live[:16]}…)")

    print("=" * 70)
    print("G5 pre-spec dataset-hash verification")
    print("=" * 70)
    ...
    if skipped and not verified:
        print(
            ...
            "vacuously passed. The integration test's M2 fixture "
            "(CI=true fails on missing) is the primary gate."
        )
...
def _update(pinned: Dict[str, Optional[str]]) -> int:
    ...
    for memo_key, (relpath, _) in ARTIFACTS.items():
        ...
        if pinned.get(memo_key) is not None:
            skipped.append(
                f"{memo_key}: already pinned ({pinned[memo_key][:16]}…); "
