"""Shard 09 Task 7: the coverage map must list ALL 46 KPIs in
config/kpi_definitions.yaml, each with a MAPPED / N/A verdict. Hermetic (files only).
Also asserts the probe script defines a path (registry or direct) for all 46."""
...
def test_coverage_map_covers_all_46_kpis():
    ids = _kpi_ids()
    assert len(ids) == 46
...
def test_probe_script_defines_path_for_all_46():
