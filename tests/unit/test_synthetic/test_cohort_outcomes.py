...
def _inputs(n=4000, seed=7):
    ...
    segment = np.where(
        disease_severity > 7, "high_severity",
...
def test_discontinuation_prevalence_in_band():
    ...
    out = generate_discontinuation_outcomes(
        rng=rng, treatment_arm=t, disease_severity=sev,
        academic_hcp=acad, segment=seg, brand_cate_scale=1.0,
    )
...
def test_brand_scale_changes_structure():
    ...
    a = generate_discontinuation_outcomes(
        rng=rng1, treatment_arm=t, disease_severity=sev,
        academic_hcp=acad, segment=seg, brand_cate_scale=0.6)
    b = generate_discontinuation_outcomes(
        rng=rng2, treatment_arm=t, disease_severity=sev,
        academic_hcp=acad, segment=seg, brand_cate_scale=1.4)
