    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GraphityConfig":
        ...
        caching = CacheConfig(
            ...
            eviction_policy=CacheEvictionPolicy(
                data.get("caching", {}).get("eviction_policy", "lru")
            ),
        ...
        hub_detection = HubDetectionConfig(
            ...
            detection_method=data.get("hub_detection", {}).get(
                "detection_method", "degree_centrality"
            ),
            update_interval_seconds=data.get("hub_detection", {}).get(
...
class GraphityConfigBuilder:
    ...
    def with_custom_pattern(
        self, name: str, pattern: str, frequency: str = "medium", description: str = ""
    ) -> "GraphityConfigBuilder":
        """Add custom traversal pattern"""
        self._traversal_patterns.append(
            {"name": name, "pattern": pattern, "frequency": frequency, "description": description}
        )
        return self
...
class GraphityOptimizer:
            ...
            if count >= threshold:
                ...
                if pattern not in [p["pattern"] for p in self.config.traversal_patterns]:
                    ...
                    report["optimizations_applied"].append(
                        {"type": "pattern_added", "pattern": pattern, "access_count": count}
                    )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_patterns": dict(
                sorted(self.access_patterns.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
...
def create_e2i_graphity_config(
    edge_chunk_size: int = 1000, cache_ttl: int = 3600, hub_threshold: int = 100
) -> GraphityConfig:
    """
