...
class TwinGenerator:
    ...
    DEFAULT_FEATURES = {
        TwinType.HCP: [
            ...
            "region", "decile", "priority_tier", "total_patient_volume",
            "target_patient_volume", "digital_engagement_score",
            "preferred_channel", "last_interaction_days", "interaction_frequency",
        ...
        TwinType.TERRITORY: [
            "region", "state_count", "zip_count", "total_hcps", "covered_hcps",
            "coverage_rate", "total_patient_volume", "market_share",
            "growth_rate", "competitor_presence"
        ],
    }
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized TwinGenerator for {twin_type.value} twins, brand={brand.value}"
        )
    ...
    def train(
        ...
        **kwargs
        ...
        # Set features
        self.feature_columns = feature_cols or self.DEFAULT_FEATURES.get(
            self.twin_type, []
        )
        ...
        logger.info(
            f"Training on {len(data)} samples with {len(self.feature_columns)} features"
        )
    ...
    def _calculate_feature_summary(
        self,
