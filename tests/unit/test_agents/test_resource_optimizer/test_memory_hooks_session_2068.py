"""#2068: resource_optimizer episodic rows must key on the REAL chat session uuid.

``store_optimization`` used to pre-hash the session id with a local
...
async def _stored_session_record(mock_supabase, session_id: str) -> dict:
    ...
    with (
        ...
        memory_id = await ResourceOptimizerMemoryHooks().store_optimization(
            session_id=session_id,
            result={
                "optimal_allocations": [],
                "objective_value": 75000.0,
                "projected_roi": 1.36,
...
@pytest.mark.asyncio
@pytest.mark.parametrize("session_id", ["", "sess_123"])
