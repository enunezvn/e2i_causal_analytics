...
# ``feature_repo/data_sources.py`` ``FROM``). This is the ONE list; it is importable
# where ``import feast`` is not (the app/worker image, #307), so the Feast tracking
# tables (#2207) and the recency probe (#559, ``_infer_source_table``) cannot drift.
...
class FeastClient:
    ...
    def _infer_source_table(self, feature_view: str) -> Optional[str]:
        ...
        mappings = {
            ...
            # the module-level FEAST_FEATURE_VIEW_SOURCE_TABLES is the single copy)
