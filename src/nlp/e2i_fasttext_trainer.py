...
CANONICAL_VOCABULARY = {
    "brands": ["Remibrutinib", "Fabhalta", "Kisqali", "ribociclib"],
    "regions": ["northeast", "southeast", "south", "midwest", "west"],
    "agents": [
        "orchestrator",
        "causal_impact",
        "gap_analyzer",
...
def train_model(corpus_path: str, model_path: str) -> fasttext.FastText._FastText:
    ...
    print("\nTraining model...")
    model = fasttext.train_unsupervised(processed_path, **TRAINING_CONFIG)
...
def find_best_match(
    model: fasttext.FastText._FastText, query: str, candidates: List[str], threshold: float = 0.5
