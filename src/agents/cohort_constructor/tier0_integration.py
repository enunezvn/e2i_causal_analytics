...
from cohort_constructor import CohortConstructor, CohortConfig, Criterion, Operator
...
class CohortConstructorAgent:
    ...
    def run(self, input_data: Dict) -> Dict:
        ...
        try:
            ...
            self._store_eligibility_log(metadata['cohort_id'], execution_id, 
                                       metadata['eligibility_log'])
            
            # Step 6: Store patient assignments
            self._store_patient_assignments(metadata['cohort_id'], execution_id, 
                                          patient_df, eligible_df)
    ...
    def _load_cohort_config(self, input_data: Dict) -> CohortConfig:
        ...
        if input_data.get('use_existing_config', False):
            ...
            response = self.supabase.table('ml_cohort_definitions')\
                .select('*')\
                .eq('brand', brand)\
    ...
    def _store_cohort_execution(self, config: CohortConfig, metadata: Dict,
                                initial_df: pd.DataFrame, eligible_df: pd.DataFrame) -> str:
        """Store cohort execution record"""
        
        execution_id = f"{config.brand}_{config.indication}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    def _store_eligibility_log(self, cohort_id: str, execution_id: str, 
                               eligibility_log: List[Dict]):
    ...
    def _store_patient_assignments(self, cohort_id: str, execution_id: str,
                                   initial_df: pd.DataFrame, eligible_df: pd.DataFrame):
...
class E2ITier0Workflow:
    ...
    def execute_tier0_pipeline(self, initial_scope: Dict) -> Dict:
        ...
        print(f"   ✓ Cohort constructed: {len(cohort_output['eligible_patients']):,} eligible patients")
