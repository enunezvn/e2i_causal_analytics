...
class TestNrxPanelDecompositionActiveRouting2141:
    @pytest.mark.parametrize(
        "query",
        [
    ...
    async def test_active_classifier_routes_the_real_profile_owner(self, query):
        ...
        assert result.routing_pattern.value == "SINGLE_AGENT", query
        assert result.target_agents == ["cohort_profiler"], query
        plan = RouterNode()._dispatch_from_classification(
            {
                "routing_pattern": result.routing_pattern.value,
                "target_agents": result.target_agents,
        ...
        assert plan is not None, query
        assert [d["agent_name"] for d in plan[0]] == ["cohort_profiler"], query
    ...
    async def test_active_classifier_does_not_claim_unsupported_continuations(self, query):
        result = await ClassificationPipeline(llm_client=None, enable_llm_layer=False).classify(
            query=query
        )
