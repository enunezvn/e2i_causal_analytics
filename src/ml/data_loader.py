...
def transform_for_supabase(table_name: str, records: List[Dict]) -> List[Dict]:
    ...
    for record in records:
        ...
        array_fields = [
            'secondary_diagnosis_codes', 'affiliation_secondary', 'comorbidities',
            'data_sources_matched', 'icd_codes', 'cpt_codes', 'loinc_codes',
            'intermediate_nodes', 'confounders_controlled', 'mediators_identified',
            'feature_list'
        ]
        ...
        jsonb_fields = [
            'lab_values', 'probability_scores', 'feature_importance', 'shap_values',
            'top_features', 'rank_metrics', 'fairness_metrics', 'features_available_at_prediction',
            'causal_chain', 'supporting_evidence', 'input_data', 'analysis_results',
            'recommendations', 'actions_initiated', 'resource_usage', 'interaction_effects',
            'source_combination_flags', 'stage_timings', 'quality_check_details',
            'annotation_value', 'adjudication_result', 'interventions_since_last',
            'feature_means', 'feature_stds', 'feature_mins', 'feature_maxs',
            'categorical_encodings', 'feature_distributions'
        ]
    ...
    try:
        ...
        for rec in records:
            try:
                ...
                print(f"    Failed record: {rec.get('patient_id', rec.get('hcp_id', 'unknown'))}: {inner_e}")
            ...
            if self.dry_run:
                ...
                for i in range(0, len(transformed), BATCH_SIZE):
                    ...
                    if (i + BATCH_SIZE) % 500 == 0:
                        print(f"  Progress: {min(i + BATCH_SIZE, len(transformed)):,}/{len(transformed):,}")
                ...
                with open(split_registry_file) as f:
                    ...
                    if split_data:
                        ...
                        if split_config_id:
                            ...
                            response = self.client.rpc('run_leakage_audit', {'p_split_config_id': split_config_id}).execute()
                            if response.data:
                                print("  Audit results:")
...
Examples:
    ...
    parser.add_argument(
        '--dry-run',
        action='store_true',
        help='Validate files without inserting data'
    )
    ...
    if not args.dry_run:
        if not SUPABASE_AVAILABLE:
            ...
            print("Error: Supabase credentials required. Set SUPABASE_URL and SUPABASE_KEY env vars.")
