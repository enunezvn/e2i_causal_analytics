...
## When Does a Query Route Here? (Routing Gate)

> Added 2026-07-29 (issue #1343). This section is the routing gate that the rest of
> this document previously left implicit. Reading only the decomposition mechanic
> below invites the rule "any query with multiple dependent steps → tool_composer" —
> that rule is **wrong**. The normative source is the classifier code cited here.

A query routes to tool_composer **only when BOTH conditions hold**:

1. **Cross-domain spanning**: the query spans **≥ 2 distinct agent-capability
   domains** (4-stage classifier: `is_multi_domain`, i.e. more than one detected
   domain), or ≥ 2 distinct strong intents in the legacy classifier. No single
   agent's contract covers the ask.
2. **Cross-domain dependency**: the sub-questions in *different* domains depend on
   each other (`is_parallelizable == False`).

What this excludes:

- **Single-domain multi-step queries are SINGLE_AGENT, never tool_composer.**
  `pattern_selector.py` Rule 3 (~L127–147) returns SINGLE_AGENT *before dependency
  analysis is consulted*. Heavy agents (causal_impact, heterogeneous_optimizer,
  cohort_profiler) run their own multi-step internal pipelines under one dispatch —
  internal multi-step-ness is not composition.
- **Multi-domain with zero dependencies is PARALLEL_DELEGATION** (Rule 5,
  ~L165–192), not tool_composer. The dependency signal's only job is to split a
  multi-domain query between PARALLEL_DELEGATION and TOOL_COMPOSER (Rule 6,
  ~L194–212).
- **Defined parallel pairs override the dependency signal**: (causal_effect,
  segment_analysis), (performance_gap, resource_allocation), (prediction,
  explanation) route PARALLEL_DELEGATION even when a sequential marker is present
  (`intent_classifier.py:432-440`, `router.py:189-192`).

Legacy-path equivalent: promotion of `primary_intent` to `multi_faceted`
(→ tool_composer) requires ≥ 2 distinct strong intents **plus** a sequential
composition marker **plus** not being a defined parallel pair
(`intent_classifier.py:429-450`; sequential-marker SSOT in `multi_faceted.py:65-108`,
which documents why a bare "then ..." from a single mapped intent must NOT promote).

Two operational caveats:

- Classifier-detected `sub_questions`/`dependencies` are **advisory only** — they
  are not passed into tool_composer, which re-decomposes from scratch
  (`router.py:451-458`).
- `multi_faceted` is a META-signal, not an agent domain (`router.py:284-298`).

Normative sources (code wins over this doc if they ever diverge):
`src/agents/orchestrator/classifier/pattern_selector.py` (Rules 1/3/5/6),
`src/agents/orchestrator/classifier/schemas.py` (`is_multi_domain`),
`src/agents/orchestrator/nodes/intent_classifier.py`,
`src/agents/orchestrator/nodes/multi_faceted.py`.
