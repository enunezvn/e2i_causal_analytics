...
Before proceeding, validate the RCA report path.

```bash
# Validate required file path
if [ -z "$1" ]; then
  echo "❌ Error: RCA report file path is required"
  echo ""
  echo "Usage: /fix-rca <path/to/RCA-report.md> [plan]"
  echo ""
  echo "Examples:"
  echo "  /fix-rca .claude/PRPs/rca/rca-report-1.md"
  echo "  /fix-rca .claude/PRPs/rca/rca-report-2.md plan"
  echo ""
  echo "Available RCA reports:"
  ls -1 .claude/PRPs/rca/*.md 2>/dev/null || echo "  (none found)"
  exit 1
fi
...
if [ ! -f "$RCA_FILE" ]; then
  echo "❌ Error: RCA report file not found: $RCA_FILE"
  echo ""
  echo "Available RCA reports:"
  ls -1 .claude/PRPs/rca/*.md 2>/dev/null || echo "  (none found)"
  exit 1
...
if [[ "$RCA_FILE" != *.md ]]; then
  echo "⚠️  Warning: RCA report should be a .md file"
  echo "Received: $RCA_FILE"
  echo ""
...
if [ -n "$2" ]; then
  if [ "$2" = "plan" ]; then
    echo "ℹ️  Mode: plan only (no implementation)"
    MODE="plan"
  else
    echo "⚠️  Warning: Unknown mode '$2' - using default (implement)"
    echo "Valid mode: 'plan' or leave blank to implement"
    MODE="implement"
  fi
else
  MODE="implement"
  echo "ℹ️  Mode: implement fix"
fi

echo "✓ Using RCA report: $RCA_FILE"
```
...
mkdir -p .claude/PRPs/rca/fixed

# Move the report to fixed folder
mv $1 .claude/PRPs/rca/fixed/
```

This keeps the `.claude/PRPs/rca/` folder clean - only unfixed issues remain there.
...
**RCA Report**: [original path] → moved to `.claude/PRPs/rca/fixed/`
