...
## ⚠️ Chat Routability — Pipeline-Only (NOT a chat routing target)

> **This contract documents the ML-pipeline API only. `cohort_constructor` can NEVER be a valid gold routing target for a chat query.** Any chat/orchestrator dispatch to this agent fails closed by design.

The request/response contract below reads like a chat/orchestrator-callable interface, but it is not. `cohort_constructor` is driven exclusively by the ML pipeline (`scope_definer → cohort_constructor → data_preparer`) via structured study parameters — never by a free-text query. Verified against the code (2026-07-29 contract review, issue #1344):

- **Real entry point**: `CohortConstructorAgent.run(patient_df, brand, indication, config, ...)` (async) / `run_sync(patient_df, brand, indication, config)` — both require a real patient `DataFrame` plus a brand/indication config. See `src/agents/cohort_constructor/agent.py:231-266` (`run`) and `516-543` (`run_sync`).
- **Absent from `INTENT_TO_AGENTS`**: no intent maps to `cohort_constructor` (`src/agents/orchestrator/nodes/router.py:44-185`). The chat form of a cohort question — "how many patients / size the cohort" — routes to the **`cohort_profiler`** companion instead (`INTENT_TO_AGENTS['cohort_definition']`, router.py:176-184).
- **Deliberately NOT in `AGENT_METHOD_MAP`**: that map is the Tier 1–5 conversational contract (`src/agents/orchestrator/_agent_method_map.py:63-69`). Without an entry, a routed chat dispatch would fall through to the default `analyze` method — which this agent does not implement.
- **Chat dispatch fails closed**: `cohort_constructor` IS a member of `VALID_AGENTS` (`src/api/routes/chatbot_dspy.py:680-693`), so a chat route *could* name it. To prevent the raw "registered but has no method 'analyze'" registry error from leaking to users (#814), the dispatcher's `_resolve_cohort_constructor_input` **always** returns `NeedsStructuredInput(missing=('patient_df','brand'))` with an actionable "use the ML cohort pipeline (scope_definer → cohort_constructor)" message, fabricating nothing. See `src/agents/orchestrator/nodes/dispatcher.py:1184-1213`. The resolver runs *before* the method lookup, and self-activates into real execution the moment the ML pipeline (not chat) supplies the structured inputs.

Everything below this callout correctly documents the **pipeline API** — the structured inputs the ML flow provides and the eligibility artifacts it returns.
