...
from src.digital_twin.models.twin_models import TwinType, Brand
...
class SimulateInterventionInput(BaseModel):
    ...
    brand: str = Field(
        description="Pharmaceutical brand ('Remibrutinib', 'Fabhalta', 'Kisqali')"
    )
    target_population: str = Field(
        default="hcp",
        description="Population type to simulate on ('hcp', 'patient', 'territory')"
    )
    
    # Intervention parameters
    channel: Optional[str] = Field(
        default=None,
        description="Communication channel ('email', 'call', 'in_person', 'digital')"
    )
    frequency: Optional[str] = Field(
        default=None,
        description="Intervention frequency ('daily', 'weekly', 'monthly')"
    )
    duration_weeks: int = Field(
        default=8,
        description="Duration of intervention in weeks"
    )
    
    # Population filters
    target_deciles: Optional[list[int]] = Field(
        default=None,
        description="Target HCP deciles (e.g., [1, 2, 3] for top 30%)"
    )
    target_specialties: Optional[list[str]] = Field(
        default=None,
        description="Target specialties (e.g., ['oncology', 'hematology'])"
    )
    target_regions: Optional[list[str]] = Field(
        default=None,
        description="Target regions (e.g., ['northeast', 'midwest'])"
    )
    
    # Simulation parameters
    twin_count: int = Field(
        default=10000,
        description="Number of digital twins to simulate (100-50000)"
    )
    confidence_level: float = Field(
        default=0.95,
        description="Confidence level for interval calculation (0.8-0.99)"
    )
...
def _create_mock_result(
    ...
    return {
        ...
        "top_segments": [
            ...
            {"dimension": "adoption_stage", "segment": "late_majority", "ate": ate * 1.15, "n": 2500},
        ],
    }
