...
- codex iteration 5: ACCEPT with 0 findings. A reviewed robustness commit followed: `_is_json_text` runs on the truncated readable value and treats RecursionError as not-JSON (a 100k-deep nested value renders truncated instead of raising); the shap rule's "top features" lookahead accepts a bare "E2I". Catalog tests 116 -> 117; readable-note tests 4 -> 5.
