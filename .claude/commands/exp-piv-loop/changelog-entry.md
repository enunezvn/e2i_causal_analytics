...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo ""
  echo "Usage: /changelog-entry [category] <description>"
  echo "Usage: /changelog-entry <description>"
  echo ""
  echo "Categories: Added, Changed, Deprecated, Removed, Fixed, Security"
  echo ""
  echo "Examples:"
  echo "  /changelog-entry \"Fix rate limiting bug\""
  echo "  /changelog-entry added \"New Discord adapter\""
  echo "  /changelog-entry fixed \"Worktree cleanup issue (#72)\""
  exit 1
fi
...
ENTRY_INPUT="$ARGUMENTS"
echo "✓ Entry: $ENTRY_INPUT"
```
