"""The registry sync client against the post-deploy schema.
...
service-role Supabase client). After one call the DB registry equals the running code: every
active tool and dependency, every field as the code declares it.

The base registry is already synced from code (#2065), so each copy first FORGETS the four tools
the lane added: that is the database a deploy meets when the code registers tools it does not
know yet, and it keeps the inserted counts meaningful. It runs at most once per process
...
#: Forgotten by every copy (with their dependencies) so the sync has real inserts to make.
FORGOTTEN = ("cohort_builder", "cohort_statistics", "cohort_validator", "model_inference")
...
def _forget(db: _pg.PgConn, names=FORGOTTEN) -> None:
    listed = ",".join(f"'{n}'" for n in names)
    db.execute(
        "delete from tool_dependencies d using tool_registry t "
        f"where t.tool_id in (d.consumer_tool_id, d.producer_tool_id) and t.name in ({listed})"
    )
    db.execute(f"delete from tool_registry where name in ({listed})")
    assert db.rows(f"select count(*) from tool_registry where name in ({listed})") == ["0"]
...
async def test_registry_and_recorder_startup_tasks_are_independent_and_never_raise(
    ...
    assert _json(migrated, "select count(*) from tool_registry where deprecated_at is null") == len(
        tools
    )
