...
**Latency**: Batch/async by design (no inherent real-time constraint) — but the orchestrator's `feedback` intent exposes a live dispatch path at a **30 s** SLA when invoked directly (see the §4.1 note on this tension)
...
**Note**: Tier 5 agents are batch/async by DESIGN and are typically NOT dispatched directly by the orchestrator in real-time:
...
> **Known tension — Feedback Learner real-time dispatch path (flagged 2026-07-29, #1344)**: despite the batch/async design above, the router DOES expose a live dispatch path for the Feedback Learner — `INTENT_TO_AGENTS['feedback'] → feedback_learner` at priority `critical`, `timeout_ms=30000` (a **30 s** SLA), no fallback (`src/agents/orchestrator/nodes/router.py:140-147`), reachable from the intent-classifier patterns `feedback | learn.*from | improve.*based on`. But `FeedbackLearnerInput` (§3.1) is **batch-shaped** — it requires `batch_id` plus `time_range_start`/`time_range_end` — so a live conversational "learn from my feedback" query has **no batch or time range to bind**. This mismatch (a real-time route pointed at a batch-only input contract) is documented here, not resolved in code.
...
|---------|------|---------|--------|
| 1.1 | 2026-07-29 | Doc-rot reconciliation (#1344): flagged the Feedback Learner real-time dispatch tension — the router exposes `feedback` → feedback_learner at critical/30 s while the input contract is batch-shaped (`batch_id` + time range), so a live "learn from my feedback" query has no batch to bind | Claude |
