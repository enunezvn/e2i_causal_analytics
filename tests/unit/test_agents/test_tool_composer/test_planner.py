...
class TestPlanCreation:
    ...
    @pytest.mark.asyncio
    async def test_plan_has_correct_structure(self, mock_llm_client, mock_tool_registry, sample_decomposition):
    ...
    @pytest.mark.asyncio
    async def test_plan_respects_dependencies(self, mock_llm_client, mock_tool_registry, sample_decomposition):
        ...
        if step_sq2 and step_sq1:
            ...
            assert step_sq1.step_id in step_sq2.depends_on_steps or len(step_sq2.depends_on_steps) > 0

    @pytest.mark.asyncio
    async def test_plan_includes_parallel_groups(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestToolMapping:
    ...
    @pytest.mark.asyncio
    async def test_tools_exist_in_registry(self, mock_llm_client, mock_tool_registry, sample_decomposition):
    ...
    @pytest.mark.asyncio
    async def test_mapping_includes_source_agent(self, mock_llm_client, mock_tool_registry, sample_decomposition):
    ...
    @pytest.mark.asyncio
    async def test_mapping_confidence_in_range(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestPlanValidation:
    ...
    @pytest.mark.asyncio
    async def test_all_sub_questions_mapped(self, mock_llm_client, mock_tool_registry, sample_decomposition):
    ...
    async def test_unknown_tool_raises_error(self, mock_llm_client, mock_tool_registry):
        ...
        mock_llm_client.set_planning_response(json.dumps({
            "reasoning": "Test",
            "tool_mappings": [
                {"sub_question_id": "sq_1", "tool_name": "unknown_tool", "confidence": 0.9, "reasoning": "Test"}
            ],
            "execution_steps": [
                {"step_id": "step_1", "sub_question_id": "sq_1", "tool_name": "unknown_tool", "depends_on_steps": []}
            ],
    ...
    @pytest.mark.asyncio
    async def test_missing_sub_question_mapping_raises_error(self, mock_llm_client, mock_tool_registry):
    ...
    async def test_step_dependency_cycle_detected(self, mock_llm_client, mock_tool_registry):
        ...
        mock_llm_client.set_planning_response(json.dumps({
            ...
            "tool_mappings": [
                {"sub_question_id": "sq_1", "tool_name": "causal_effect_estimator", "confidence": 0.9, "reasoning": "Test"},
                {"sub_question_id": "sq_2", "tool_name": "cate_analyzer", "confidence": 0.9, "reasoning": "Test"}
            ],
            "execution_steps": [
                {"step_id": "step_1", "sub_question_id": "sq_1", "tool_name": "causal_effect_estimator", "depends_on_steps": ["step_2"]},
...
class TestDurationEstimation:
    ...
    @pytest.mark.asyncio
    async def test_duration_estimated(self, mock_llm_client, mock_tool_registry, sample_decomposition):
    ...
    @pytest.mark.asyncio
    async def test_duration_based_on_tool_schemas(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestToolDescriptionFormatting:
    ...
    @pytest.mark.asyncio
    async def test_tools_included_in_prompt(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestEmptyRegistryHandling:
    ...
    @pytest.mark.asyncio
    async def test_empty_registry_raises_error(self, mock_llm_client, empty_registry, sample_decomposition):
...
class TestInputMappingParsing:
    ...
    @pytest.mark.asyncio
    async def test_input_mapping_preserved(self, mock_llm_client, mock_tool_registry, sample_decomposition):
        ...
        has_input_mapping = any(
            len(step.input_mapping) > 0
            for step in plan.steps
        )
        assert has_input_mapping

    @pytest.mark.asyncio
    async def test_step_reference_syntax(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestResponseParsing:
    ...
    @pytest.mark.asyncio
    async def test_parse_markdown_code_block(self, mock_llm_client, mock_tool_registry, sample_decomposition):
        """Test parsing JSON from markdown code block"""
        mock_llm_client.set_planning_response("""Here is the plan:
...
```""")
    ...
    @pytest.mark.asyncio
    async def test_invalid_json_raises_error(self, mock_llm_client, mock_tool_registry, sample_decomposition):
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_llm_error_wrapped(self, mock_llm_client, mock_tool_registry, sample_decomposition):
