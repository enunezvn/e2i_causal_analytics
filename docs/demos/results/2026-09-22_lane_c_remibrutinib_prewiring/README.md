...
Consequence for the design: the vocabulary matches the NDC at PRODUCT level only (the bare labeler is refused by a test) and the names decide on every real row measured.

**Marketed NDC (verifier MED-C, 2026-09-22).** The marketed Rhapsido NDC is public, so "owner addition at the post-launch refresh" was the wrong disposition. Verified against openFDA on 2026-09-22 (`curl api.fda.gov/drug/ndc.json?search=generic_name:remibrutinib`): product_ndc `0078-1483`, brand RHAPSIDO, labeler Novartis Pharmaceuticals Corporation, 25 mg tablet, packages `0078-1483-20` / `-92` / `-93`; the placeholder `0078-1100` is `NOT_FOUND` (no real product can be misclassified by it). The same query also lists `76047-003` (Lianhe Chemical, bulk API powder, 18 kg) — an API manufacturer's listing, not a dispensed product, deliberately NOT added. `src/data/csu_biologics.py` now carries the marketed product in every form a claim code can take — 11-digit dashless `000781483` (the drop's form), dashed 5-4 `00078-1483`, dashed 4-4 `0078-1483` (openFDA's print form) and the raw dashless 10-digit 4-4-2 `00781483` — ahead of the placeholder (`000781100` / `00078-1100`, the two forms the synthetic claims generator emits; no raw 10-digit form exists for a code only our generator produces). `test_convert_optum_rwd_remibrutinib.py` pins every form through both the vocabulary and the converter and still refuses the bare labeler (`00078` / `0078`). One theoretical collision is recorded, not silenced: an 8-digit raw prefix `00781483` would also match an 11-digit code on labeler `00781` (Sandoz) with product code `483x`; openFDA lists no such product today (`search=product_ndc:0781-483*` → NOT_FOUND), and Dupixent's `0024` prefix has carried the same class of ambiguity since #157.
...
## D2b — does the recovery check discriminate the payer backdoor? (verifier MED-A)

The verifier's plant E (payer_category removed from the CI adjustment set, its two column-presence assertions deleted) PASSED the shipped E2E: the spec's sidecar tolerance (0.10) admits losing the whole planted payer confounder, so the payer path was asserted only by a proxy (`"payer_category=medicare" in task_covariates`). Cheapest disproof of any replacement threshold: measure what the REAL graph returns with and without the payer backdoor, then place the threshold between them. `d2b_graph_payer_lost_measure.jsonl` (this directory; the two E2E variants run in-process, write-free, under the box lock) and the extended `d2_estimator_vs_dgp_recovery.txt`:

| estimate on the E2E frame (n = 3,000, seed 20260922; truth 0.364, naive 0.4976) | ATE | error | cite |
|---|---|---|---|
| graph LinearDML, full CI adjustment (age, charlson, payer one-hot + 5 nuisance) — run 1 | 0.40622 | 0.0422 | `d2b_graph_payer_lost_measure.jsonl:1` |
| graph LinearDML, full CI adjustment — run 2 (repeat, same harness) | 0.40622 (identical to 15 decimals) | 0.0422 | `d2b_graph_payer_lost_measure.jsonl:3` |
| graph LinearDML, payer_category DROPPED (plant E) | 0.43641, CI [0.404, 0.469] | 0.0724 | `d2b_graph_payer_lost_measure.jsonl:2` |
| OLS constant-effect, full adjustment | 0.3845 | 0.0205 | `d2_estimator_vs_dgp_recovery.txt:3,5` |
| OLS without payer_category | 0.4315 | 0.0675 | `d2_estimator_vs_dgp_recovery.txt:4,5` |
| naive diff-in-means | 0.4976 | 0.1336 | `d2_estimator_vs_dgp_recovery.txt:5` |

The D2 probe harness (`d2_probe_narrow_3000.json`) gave 0.4072 on the same shape; the E2E harness is deterministic on this box (two runs identical), so the run-to-run spread is ≤ 0.001 across harnesses.

**The check that ships** (`PlantedTruth.is_recovery_convincing`, `src/ml/synthetic/generators/csu_escalation_causal.py`): error < `RECOVERY_TOLERANCE` = 0.06 AND error < 0.5 × naive error (= 0.0668 here). 0.06 is the midpoint of the two measured graph outcomes (0.042 and 0.072 → 0.057, rounded up): the full adjustment passes with a 0.018 margin, the payer-lost run fails by 0.012, both ≥ 12× the measured spread. The lead's first proposal (0.05) would also separate them (0.008 / 0.022) but skews the margin toward the pass side; the midpoint maximises the minimum margin. The sidecar tolerance (0.10) is still asserted as the spec's criterion; it is no longer the discriminating one. Teeth re-proven after the change (`gates_summary.txt`): plant C (adjustment set emptied) still fails on the naive 0.4976; plant E (payer dropped) now FAILS on `is_recovery_convincing` (error 0.0724 > 0.06); the clean E2E passes.

**Observation, not a fix (follow-up).** With the adjustment set emptied (plant C) the refutation suite still reported "5/5 passed, E-value 5.62 robust" on the NAIVE estimate: the refutation node re-fits whatever adjustment set it is handed and cannot tell that the set is wrong — a pre-existing property of the node, recorded here for the estimator-calibration follow-up alongside the LinearDML +0.04 bias.
...
- The synthetic backing rows are NOT loaded into prod `csu_escalation_causal` (needs the loader generalised from Lane A's `load_optum_causal_cohort.py`, now on main, and an explicit GO). **Deployed-flag fact (verifier MED-B):** the prod `e2i_api` container runs with `E2I_INCLUDE_SYNTHETIC=true` (`docker inspect e2i_api`, `.env:105`, re-verified 2026-09-22), which makes `apply_provenance_filter` a no-op for every reader — so before this fix the load would have made prod serve planted rows as if real. The dataset is now in `_CAUSAL_SYNTHETIC_BACKED` (`src/api/routes/causal/datasets.py`): the agent loader, the estimation-data route and the brand dropdown apply the real-mode `is_synthetic = false` predicate REGARDLESS of that flag; only the `PLANTED_TRUTH_RUN` module seam (flipped by the E2E with `monkeypatch.setattr` for its own process — a module attribute, not an environment variable, so nothing in a container manifest, config map or parent process can set it; codex r1 MED replaced the first cut's `E2I_CSU_PLANTED_TRUTH_RUN` env var with it) reads the rows, and then reads ONLY `is_synthetic = true` rows (never an unfiltered mixture labelled synthetic; codex r1 HIGH); the response's `data_source` label follows the same rule on the agent, discovery-leaderboard and estimation routes, and the `/causal/variables` one-row schema probe is guarded too (codex r1 MED). **The synthetic load is NOT to be run until this guard is on main.** Until loaded, real mode AND the deployed instance return 503 for the dataset; the E2E certifies the wiring from the generator's rows.
- The marketed Rhapsido NDC (`0078-1483`, openFDA-verified) IS carried (D-NDC); the spec-named placeholder (`00078-1100`) stays alongside it for the synthetic claims generator. Nothing is pending from the owner here; the post-launch refresh only needs to confirm the package codes on the first real Rhapsido fills.
- **Spec item 3 substitution (verifier MED-D).** Spec §3 Lane C item 3 names "the synthetic CSU cohort (`data/rwd/synthetic_CSU`, Remibrutinib brand, planted confounders) run through the whole Lane A + B path". Shipped instead: (i) a NEW generator in the `csu_escalation_causal` contract (`src/ml/synthetic/generators/csu_escalation_causal.py`) — the on-disk `synthetic_CSU` set (Remibrutinib-filtered cohort frames + `ground_truth_20260610T211458.json`) is in the e2i_ml_v3 `patient_journeys` shape (initiation / persistence frames, `treatment_arm`, journey covariates), not the 64-feature claims contract the registry entry and migration 149 declare, so it cannot flow through the Lane A loader without a converter that would itself be new code; the generator plants the same kind of truth in the contract's own shape; (ii) the main-branch path only — curated common-cause DAG (`dag_source = domain_knowledge`), no Lane B attestation / `anchored_confounders` leg, because Lane B is unmerged; (iii) the CI run shape is the narrow width (D2); the full width is behind `E2I_CSU_PLANTED_FULL_WIDTH=1` until Lane A's identify fix is exercised on this dataset.
- The LinearDML +0.04 bias with a CI excluding the truth on this DGP (D2), and the refutation suite's inability to notice a lost adjustment set (D2b observation), are recorded estimator-calibration follow-ups.
