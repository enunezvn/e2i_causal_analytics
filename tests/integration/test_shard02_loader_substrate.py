...
pytestmark = pytest.mark.skipif(os.getenv("E2I_DB_INTEGRATION") != "1", reason="faithful DB only")
...
def test_shard02_substrate_lands_in_docker_supabase():
    ...
    assert (
        ...
        == 3
    ), "ml_predictions causal columns must exist (Shard 01 DDL)"
    ...
    if "permission denied" in combined or "0 loaded" in combined:
        pytest.skip(
            "loader write path is not authorized in this environment (permission denied / 0 loaded)"
        )

    # business_metrics: gap-connector key present + ONLY the 5 lowercase keys.
    assert (
        _psql_count(
            "SELECT count(*) FROM business_metrics WHERE metric_name='trx' AND is_synthetic=true;"
        )
        > 0
    )
    assert (
        _psql_count(
            "SELECT count(DISTINCT metric_name) FROM business_metrics WHERE is_synthetic=true;"
        )
        == 5
    )
    # provenance stamp survived the loader.
    assert _psql_count("SELECT count(*) FROM ml_predictions WHERE is_synthetic=true;") > 0
