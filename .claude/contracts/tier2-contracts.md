...
**Latency**: Router dispatch SLA is **30s** (`causal_effect` intent → `timeout_ms=30000`, `fallback_agent=explainer`; `router.py:45-53`). An earlier "up to 120s (60s computation + 30s interpretation)" figure predated the router timeout and never matched the actual dispatch; the 30s dispatch timeout is operative (discrepancy reconciled 2026-07-29).
**Pipeline**: `audit_init` (genesis) → `graph_builder` (build the causal DAG; optionally runs V4.4 automatic structure learning when `auto_discover=True` — see the Discovery Configuration input fields below) → `adjustment_set_policy` (collider/mediator exclusion policy, Issue #237; OFF by default) → `estimation` (DoWhy/EconML ATE) → `refutation` (conditional — a *total* estimation failure with no ATE routes to the error handler instead; a partial success is still validated here) → `sensitivity` (E-value) → `interpretation` (NL narrative) → END, with an `error_handler` branch (`graph.py:447-497`). All one SINGLE_AGENT dispatch. The default `estimation_method` is `backdoor.econml.dml.CausalForestDML`, but the DAG itself may be auto-discovered rather than supplied.
...
**Latency**: Up to 20s (`performance_gap` intent → `timeout_ms=20000`, no fallback)
**Pipeline**: `audit_init` (genesis) → `gap_detector` → `roi_calculator` → `instrument_analyzer` (#357 — computes per-feature instrument strength from a real IV first-stage F-test, feeding the prioritizer's instrument-availability bonus) → `prioritizer` → `formatter` → END, linear, one SINGLE_AGENT dispatch (`graph.py:88-111`).
...
class GapAnalyzerOutput(BaseModel):
    ...
    steady_plays: List[PrioritizedOpportunity] = Field(
        ...,
        description="Middle bucket of the T6 3-bucket partition (moderate ROI/difficulty; neither quick win nor strategic bet)"
    )
    ...
    suppressed_count: int = Field(
        0,
        description="Count of value-destroying opportunities (ROI <= break-even) hidden from the buckets; distinguishes a deliberate 'nothing worth recommending' completion from an empty/malfunctioning run"
    )
...
**Latency**: Router dispatch SLA is **420s / 7 min** (`segment_analysis` intent → `timeout_ms=420000`, `fallback_agent=gap_analyzer`; `router.py:63-85`), raised for real CausalForestDML runs on the full frame — the router comment documents a measured 269.7s serialized run. An earlier "up to 150s" figure is stale (discrepancy reconciled 2026-07-29).
**Pipeline**: `audit_init` (genesis) → `estimate_cate` (CausalForestDML CATE + heterogeneity_score + feature_importance) → `analyze_segments` (high/low responders) → [`hierarchical_analysis` (CausalML hierarchical uplift; feature-flagged, optional)] → [`uplift_analysis` (optional, non-fatal — an uplift failure degrades to a warning so CATE/responder/policy outputs survive)] → `learn_policy` (allocation policy) → `generate_profiles` (viz + insights) → END, with an `error_handler` branch (`graph.py:173-237`). One SINGLE_AGENT dispatch.
...
> **Confidence SSOT (dual callsite)**: `confidence` is computed by a module-level `calculate_confidence(state)` (`heterogeneous_optimizer/agent.py:29`), not by a graph node. Two callsites share it: `HeterogeneousOptimizerAgent._build_output` (the `agent.run()` path) and the FastAPI `/segments` route, which invokes the graph directly (`graph.ainvoke`, `segments.py:1781`) and therefore never runs `_build_output`. Because no graph node writes `confidence` into state, the route must call `calculate_confidence(result)` itself (`segments.py:1843`); keeping the function module-level guarantees both paths score confidence identically.
