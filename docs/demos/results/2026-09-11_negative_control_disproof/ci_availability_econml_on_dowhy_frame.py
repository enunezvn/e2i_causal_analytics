...
for method in [
    ...
    for t, o, cc in pairs:
        ...
        inner = est.estimator
        econ = inner.estimator
        attrs = [a for a in dir(inner) if "modif" in a.lower()]
        Xd = getattr(inner, "_effect_modifiers", None)
        info = f"type={type(Xd).__name__}" + (
            f" shape={getattr(Xd, 'shape', None)} cols={list(Xd.columns) if hasattr(Xd, 'columns') else None} dtypes={[str(x) for x in Xd.dtypes] if hasattr(Xd, 'dtypes') else None}"
            if Xd is not None
            else ""
        )
        t0 = time.time()
        try:
            ...
            print(
                f"{method.split('.')[-1]:15} {t}->{o}: dowhy={float(est.value):+.4f} econml_on_dowhy_X={float(inf.mean_point):+.4f} ci=[{lo:+.4f},{hi:+.4f}] {time.time() - t0:.2f}s | {info} | attrs={attrs}"
            )
        except Exception as e:
            print(
                f"{method.split('.')[-1]:15} {t}->{o}: FAILED {type(e).__name__}: {str(e)[:120]} | {info} | attrs={attrs}"
            )
