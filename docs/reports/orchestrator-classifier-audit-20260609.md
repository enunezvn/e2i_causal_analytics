...
Implemented the three live-engine fixes for **C2** (multi-part queries collapsing to a single agent) and **C3** (the `requires_multi_agent` secondary intent being silently dropped). Branch `feat/orchestrator-multipart-routing`; TDD red-first; tests in
`tests/unit/test_agents/test_orchestrator/test_multipart_tool_composer_routing.py` (23 cases) — all 351 orchestrator unit tests green, no mocks.

**Design principle (the discriminator):** route to `tool_composer` only for **dependent pipelines** — an explicit sequential/dependency marker ("then", "after that", "based on that", …) joining ≥2 analytical asks. Independent/compound asks ("compare X and Y", "what causes A and what drives B and also explain C") have *no* such marker and stay single-/parallel-agent. This matches the original 4-stage design's intent (TOOL_COMPOSER only on detected dependencies), `tool_composer/decomposer.py` (which owns the real LLM decomposition + dependency DAG — so the orchestrator only needs to *detect* "multi-part, hand off", not decompose), and the locked `near-miss-single-regex-hit-loses-on-score` test.

| Fix | File | What it does |
|---|---|---|
| **1 — router safety net** | `nodes/router.py` | When `requires_multi_agent` but the (primary, secondary) pair isn't a hard-coded `MULTI_AGENT_PATTERNS` parallel pair: parallel-delegate **primary + top real-domain secondary** instead of silently dropping the secondary (C3). The `multi_faceted` meta-signal as a *secondary* is skipped (it only routes when it's the *primary*). A classifier-promoted `multi_faceted` primary falls through to single-agent dispatch → `tool_composer`. |
| **2 — classifier pipeline promotion** | `nodes/intent_classifier.py` + `multi_faceted.has_sequential_composition` | A sequential/dependency marker joining ≥2 distinct strong intents promotes the primary intent to `multi_faceted` (→ `tool_composer`). Guarded to an explicit marker so additive compounds stay single-agent. |
| **3 — broaden detection + LLM backstop** | `multi_faceted.MULTI_FACETED_PATTERNS` (4→6) + `intent_classifier._needs_llm_disambiguation` | Two new SSOT patterns for natural sequential forms (`", and <wh> … then"`, `"then <verb>"`, both requiring a "then"-marker so the locked single-intent negatives can't flip). Plus a deterministic predicate that escalates borderline soft-signal queries (facet/topic scorers, sequence marker) to the existing Haiku fallback rather than short-circuiting to one agent. |

**Still open (not in this change, flagged so they are not lost):**
- **C1** — `"what drove …"` (past tense) still misroutes to `general`/explainer because the causal pattern token `driv` does not match `drove`. One-line regex fix; recommended as a fast follow-up.
- **C5** — verify the project-wide fast-model id `claude-haiku-4-20250414`.
- **A1/B-series** — the orphaned 4-stage `classifier/` package remains unwired (Option A/B/C decision still open). This change deliberately improves the **live** engine and does **not** wire the orphan.
