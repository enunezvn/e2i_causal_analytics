...
if [ -z "$ARGUMENTS" ]; then
  ...
  echo ""
  echo "Usage: /prp:prp-core-create <feature-description>"
  echo ""
  echo "Examples:"
  echo "  /prp:prp-core-create \"Add user authentication system\""
  echo "  /prp:prp-core-create \"Implement search functionality with filters\""
  exit 1
fi

# Check minimum length for meaningful description
if [ ${#ARGUMENTS} -lt 10 ]; then
  echo "❌ Error: Feature description too short (minimum 10 characters)"
  echo "Provide a clear, descriptive feature name"
  exit 1
fi
