...
Before proceeding, validate the input arguments if provided.

```bash
# Optional argument - defaults to latest unreleased
if [ -z "$ARGUMENTS" ]; then
  echo "ℹ️  No version/tag specified, will use unreleased changes or latest tag"
  SOURCE=""
else
  # Could be version number, tag name, or "since <tag>"
  # Accept any format - we'll validate during processing
  SOURCE="$ARGUMENTS"
...
```bash
mkdir -p .claude/PRPs/releases
```

Save to: `.claude/PRPs/releases/v0.3.0-notes.md`
...
Or manually:
gh release create v0.3.0 --title "v0.3.0" --notes-file .claude/PRPs/releases/v0.3.0-notes.md
```
