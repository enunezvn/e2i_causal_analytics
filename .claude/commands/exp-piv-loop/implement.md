...
Before proceeding, validate the plan file argument.

```bash
# Validate required file path
if [ -z "$1" ]; then
  echo "❌ Error: Plan file path is required"
  echo ""
  echo "Usage: /implement <path-to-plan.md>"
  echo ""
  echo "Example: /implement .claude/PRPs/features/active/feature-name.plan.md"
  echo ""
...
if [[ "$PLAN_FILE" != *.md ]]; then
  echo "⚠️  Warning: Plan file should be a .md file"
  echo "Received: $PLAN_FILE"
  echo ""
...
**Plan**: [path] → moved to `.claude/PRPs/features/completed/`
...
```bash
mkdir -p .claude/PRPs/features/active/completed
mv $1 .claude/PRPs/features/completed/
```
