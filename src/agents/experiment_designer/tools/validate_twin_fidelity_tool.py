...
class ValidateFidelityInput(BaseModel):
    ...
    simulation_id: str = Field(description="UUID of the original twin simulation")
    actual_ate: float = Field(description="Actual Average Treatment Effect from real experiment")
    actual_ci_lower: Optional[float] = Field(
        default=None, description="Lower bound of actual confidence interval"
    )
    actual_ci_upper: Optional[float] = Field(
        default=None, description="Upper bound of actual confidence interval"
    )
    actual_sample_size: Optional[int] = Field(
        default=None, description="Sample size of actual experiment"
    )
    actual_experiment_id: Optional[str] = Field(
        default=None, description="ID of the actual experiment in ml_experiments table"
    )
    validation_notes: Optional[str] = Field(
        default=None, description="Optional notes about the validation"
    )
...
def validate_twin_fidelity(
    ...
    try:
        ...
        try:
            ...
            return _create_error_response(f"Invalid simulation_id format: {simulation_id}")
