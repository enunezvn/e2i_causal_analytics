-- ============================================================================
-- E2I CAUSAL ANALYTICS — hybrid search default-excludes synthetic episodic rows
-- ============================================================================
-- Migration: 044_hybrid_search_exclude_synthetic.sql  (ledger label MEM1)
-- Purpose: Shard 07 (Provenance Read-Path Enforcement) Gate 9 (RAG leg, R13).
--          The prod chatbot must NEVER surface synthetic KPI prose. corpus_ingestion
--          (R12) already skips synthetic business_metrics at ingest; this is the SQL
--          backstop so even a leaked synthetic episodic_memories row is filtered out
--          of hybrid_vector_search / hybrid_fulltext_search by default. Validation /
--          eval callers opt in via filters->>'include_synthetic' = 'true'.
--
--   Both functions are re-created VERBATIM from 043_hybrid_search_operational_corpus
--   .sql; the ONLY change is one predicate added to the episodic_memories (em) branch
--   of each function's WHERE clause:
--       AND (COALESCE(filters->>'include_synthetic','false') = 'true'
--            OR em.is_synthetic = false)
--   episodic_memories.is_synthetic is Shard 01's BOOLEAN NOT NULL DEFAULT false
--   column (partial index idx_episodic_memories_is_synthetic). All other branches
--   (procedural_memories, causal_paths, agent_activities, triggers) are unchanged —
--   those tables are not part of the synthetic corpus blast radius.
--
-- Dependencies: 043_hybrid_search_operational_corpus.sql (the bodies copied here),
--               Shard 01 (episodic_memories.is_synthetic column).
-- Idempotent: CREATE OR REPLACE (no signature change); safe to re-apply.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- PART 1 — dense RPC: default-exclude synthetic on the episodic branch (R13)
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    WHERE
        ...
        -- Provenance (migration 044 / Shard 07 R13): default-exclude synthetic;
        -- opt in via filters->>'include_synthetic'='true'.
        AND (COALESCE(filters->>'include_synthetic','false') = 'true' OR em.is_synthetic = false)
...
'Semantic search across episodic_memories (incl. the operational KPI corpus) and procedural_memories using pgvector cosine similarity. brand/region filters are case-insensitive (migration 043, audit F3a). Default-excludes synthetic episodic rows; opt in via filters->>''include_synthetic''=''true'' (migration 044, Shard 07 R13). Returns top matches with metadata.';

GRANT EXECUTE ON FUNCTION hybrid_vector_search TO authenticated;

-- ----------------------------------------------------------------------------
-- PART 2 — sparse RPC: default-exclude synthetic on the episodic branch (R13),
...
-- ROLLBACK
-- ============================================================================
-- Re-apply 043_hybrid_search_operational_corpus.sql to restore the previous
-- (synthetic-inclusive) episodic branches.
-- ============================================================================
