...
Before proceeding, validate the user request.

```bash
# Required natural language request
if [ -z "$ARGUMENTS" ]; then
  echo "❌ Error: User request is required"
  echo ""
  echo "Usage: /exp-piv-loop:router <natural-language-request>"
  echo ""
  echo "This command is typically called automatically when a user mentions @remote-agent"
  exit 1
fi

USER_REQUEST="$ARGUMENTS"
echo "✓ Analyzing request: $USER_REQUEST"
```
