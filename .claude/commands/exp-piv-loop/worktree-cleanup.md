...
Before proceeding, validate the cleanup target.

```bash
# Check if target is provided
if [ -z "$1" ]; then
  echo "❌ Error: Cleanup target is required"
  echo ""
  echo "Usage: /worktree-cleanup <branch-name|all|merged> [--delete-branches]"
  echo ""
  echo "Examples:"
  echo "  /worktree-cleanup feature/discord-adapter"
  echo "  /worktree-cleanup all --delete-branches"
  echo "  /worktree-cleanup merged"
  exit 1
fi
...
if [ -n "$2" ]; then
  if [ "$2" = "--delete-branches" ]; then
    ...
    echo "ℹ️  Will delete local and remote branches"
  else
    echo "⚠️  Warning: Unknown option '$2' - ignoring"
    echo "Valid option: --delete-branches"
  fi
fi

# Validate target format
if [ "$TARGET" = "all" ]; then
  echo "✓ Target: Clean up all worktrees"
elif [ "$TARGET" = "merged" ]; then
  echo "✓ Target: Clean up only merged worktrees"
else
  echo "✓ Target: Clean up worktree for branch '$TARGET'"
fi
```
