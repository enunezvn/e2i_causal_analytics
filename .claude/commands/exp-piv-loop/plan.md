...
if [ -z "$ARGUMENTS" ]; then
  echo "❌ Error: Feature description or PRD path is required"
  echo ""
  echo "Usage: /plan <feature-description>"
  echo "Usage: /plan <path-to-prd.md>"
  echo ""
  echo "Examples:"
  echo "  /plan \"Add user authentication\""
  echo "  /plan .claude/PRPs/features/active/user-auth-prd.md"
  exit 1
fi
...
if [[ "$ARGUMENTS" == *.md ]]; then
  ...
  if [ ! -f "$ARGUMENTS" ]; then
    echo "❌ Error: PRD file not found: $ARGUMENTS"
    echo ""
    echo "Available PRD files:"
    find .claude/PRPs -name "*.md" -type f 2>/dev/null | head -5 || echo "  (none found)"
    exit 1
  fi
  FEATURE_INPUT="$ARGUMENTS"
  echo "✓ Using PRD file: $FEATURE_INPUT"
else
  # It's a feature description
  # Check minimum length
  if [ ${#ARGUMENTS} -lt 5 ]; then
    echo "❌ Error: Feature description too short (minimum 5 characters)"
    exit 1
  fi
  FEATURE_INPUT="$ARGUMENTS"
  echo "✓ Feature description: $FEATURE_INPUT"
fi
```
...
- `.claude/reference/new-features.md` - If adding new feature type
...
**Save to**: `.claude/PRPs/features/active/{feature-name}.plan.md`

Create `.claude/PRPs/features/active/` if it doesn't exist: `mkdir -p .claude/PRPs/features/active`
...
**File**: `.claude/PRPs/features/active/{feature-name}.plan.md`
...
**To implement**: `/implement .claude/PRPs/features/active/{feature-name}.plan.md`
```
