...
class GraphPath(BaseModel):
    ...
    relationships: List[GraphRelationship] = Field(..., description="Relationships connecting nodes")
...
class ListNodesRequest(BaseModel):
    ...
    search: Optional[str] = Field(
        None,
        max_length=500,
        description="Text search in node names/properties"
    )
    filters: Optional[Dict[str, Any]] = Field(
        None,
        description="Property filters (e.g., {\"region\": \"Northeast\"})"
    )
...
class TraverseRequest(BaseModel):
    ...
    min_confidence: float = Field(default=0.0, ge=0.0, le=1.0, description="Minimum edge confidence")
...
class GraphSubscription(BaseModel):
    ...
    relationship_types: Optional[List[RelationshipType]] = Field(
        None,
        description="Relationship types to subscribe to"
    )
    session_ids: Optional[List[str]] = Field(
        None,
        description="Session IDs to filter events"
    )
