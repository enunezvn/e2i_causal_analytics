...
class TestAnalystEndpoints:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                "post",
                {"treatment": "test", "outcome": "test", "data_source": "test"},
            ),
            ("/api/causal/route", "post", {"query": "test query"}),
            (
                "/api/causal/pipeline/sequential",
                "post",
                {"stages": [], "treatment": "test", "outcome": "test"},
            ),
            (
                "/api/causal/pipeline/parallel",
                "post",
                {"libraries": [], "treatment": "test", "outcome": "test"},
            ),
            (
                "/api/causal/validate",
                "post",
                {"treatment": "test", "outcome": "test", "libraries": []},
            ),
    ...
    def test_viewer_denied_analyst_endpoints(self, app_client, endpoint, method, body):
        ...
        assert response.status_code == 403, (
            f"Expected 403 for {endpoint}, got {response.status_code}: {response.text}"
        )
    ...
    def test_analyst_allowed_analyst_endpoints(self, app_client, endpoint, method, body):
        ...
        assert response.status_code not in [401, 403], (
            f"Unexpected auth error for {endpoint}: {response.text}"
        )
