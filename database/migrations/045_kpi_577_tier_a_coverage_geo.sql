...
--     (hcp_profiles has no brand column, so the optional $1 brand param bands
--     only the universe denominator — documented as a proxy in the note.)
...
INSERT INTO public.kpi_query_registry (query_id, sql, max_params, note) VALUES
    ('data_quality_source_coverage_hcps', $kpi$SELECT (SELECT COUNT(DISTINCT hcp_id) FROM hcp_profiles WHERE coverage_status = true) AS covered, COALESCE((SELECT SUM(target_count) FROM reference_universe WHERE universe_type = 'hcp' AND ($1::text IS NULL OR brand::text = $1)), 0) AS total$kpi$, 1, $note$PROXY: covered = distinct HCPs with coverage_status=true (hcp_profiles has no brand column, so the numerator is brand-agnostic); denominator = SUM(reference_universe.target_count) for universe_type='hcp', optionally brand-banded by $1. Mirrors merged DQ-001 (data_quality_source_coverage_patients).$note$),
