...
class SimulationResult(BaseModel):
    ...
    # Heterogeneity
    effect_heterogeneity: EffectHeterogeneity = Field(
        default_factory=EffectHeterogeneity
    )
...
class FidelityRecord(BaseModel):
    ...
    def calculate_fidelity(self) -> None:
        ...
        self.prediction_error = (
            (self.simulated_ate - self.actual_ate) / self.actual_ate
            if self.actual_ate != 0 else 0
        )
        ...
        if self.simulated_ci_lower is not None and self.simulated_ci_upper is not None:
            self.ci_coverage = (
                self.simulated_ci_lower <= self.actual_ate <= self.simulated_ci_upper
            )
