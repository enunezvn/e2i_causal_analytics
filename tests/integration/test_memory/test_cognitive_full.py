...
def mock_working_memory():
    ...
    memory.get_session = AsyncMock(
        return_value={"user_id": "user_001", "context": {"brand": "Kisqali"}, "state": "active"}
    )
...
def mock_memory_functions():
    ...
    with (
        patch("src.memory.cognitive_integration.insert_episodic_memory_with_text") as mock_episodic,
...
class TestCognitiveService:
    ...
    async def test_process_query_completes_all_phases(
        self, mock_working_memory, mock_hybrid_search, mock_memory_functions
    ):
        """Should complete all 4 cognitive phases."""
        with patch(
            "src.memory.cognitive_integration.get_working_memory", return_value=mock_working_memory
        ):
            service = CognitiveService()
            service._working_memory = mock_working_memory

            input_data = CognitiveQueryInput(
                query="Why did TRx drop in the northeast?", brand="Kisqali", region="northeast"
            )
    ...
    async def test_query_type_detection_causal(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(
                query="Why did adoption rates change?", brand="Kisqali"
            )
    ...
    async def test_query_type_detection_prediction(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(
                query="What will TRx be next quarter?", brand="Kisqali"
            )
    ...
    async def test_query_type_detection_optimization(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(
                query="How can we optimize resource allocation?", brand="Kisqali"
            )
    ...
    async def test_evidence_included_when_requested(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(query="Why did TRx drop?", include_evidence=True)
    ...
    async def test_evidence_excluded_when_not_requested(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(query="Why did TRx drop?", include_evidence=False)
    ...
    async def test_confidence_calculation(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(query="Why did TRx drop?", brand="Kisqali")
    ...
    async def test_session_reuse(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(
                query="What else affects this?", session_id="existing_session_123"
            )
    ...
    async def test_processing_time_tracked(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(query="Quick question")
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, mock_working_memory, mock_memory_functions):
    ...
    async def test_process_cognitive_query(
        ...
        with patch(
            ...
            with patch(
                ...
                result = await process_cognitive_query(query="Why did TRx drop?", brand="Kisqali")
...
class TestVisualizationConfig:
    ...
    async def test_causal_query_gets_sankey(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(query="Why did this happen?", include_evidence=True)

            result = await service.process_query(input_data)

            if result.visualization_config:
                assert result.visualization_config.get("chart_type") in [
                    "sankey",
                    "bar",
                    "line",
                    "table",
    ...
    async def test_prediction_query_gets_line(
        ...
        with patch(
            ...
            input_data = CognitiveQueryInput(
                query="What will happen next quarter?", include_evidence=True
            )
