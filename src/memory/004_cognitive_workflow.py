...
async def extract_entities(query: str) -> Dict[str, List[str]]:
    ...
    for category, kpis in vocab["entity_types"]["kpi"]["categories"].items():
        for kpi in kpis:
    ...
    while state["current_hop"] < state["max_hops"] and not state["investigation_complete"]:
        ...
        if state["current_hop"] == 1:
            ...
            for result in episodic_results:
                hop_evidence.append(EvidenceItem(
                    hop_number=1,
                    source="episodic",
...
def select_top_evidence(evidence_trail: List[EvidenceItem], top_k: int = 10) -> List[EvidenceItem]:
    ...
    for i, e in enumerate(sorted_evidence[:top_k]):
        e.selected = True
...
async def agent_node(state: CognitiveState) -> CognitiveState:
    ...
    state["agents_to_invoke"] = intent_to_agents.get(
...
Response:"""
    ...
    if selected_evidence:
        state["confidence_score"] = sum(e.relevance_score for e in selected_evidence) / len(selected_evidence)
    else:
...
async def generate_viz_config(intent: str, agent_outputs: Dict[str, Any]) -> Dict[str, Any]:
    ...
    viz_types = {
        "causal_intents": "sankey",
        "trend_intents": "line",
        "comparison_intents": "bar"
    }
...
Triplets (one per line, or NONE if no new facts):"""
    ...
    state["feedback_signals"] = [{
        "signal_type": "outcome_success" if state["confidence_score"] > 0.7 else "outcome_partial",
