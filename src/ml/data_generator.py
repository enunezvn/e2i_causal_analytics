...
EVENT_TYPES = ['diagnosis', 'prescription', 'lab_test', 'procedure', 'consultation', 'hospitalization']
PREDICTION_TYPES = ['trigger', 'propensity', 'risk', 'churn', 'next_best_action']
JOURNEY_STAGES = ['diagnosis', 'initial_treatment', 'treatment_optimization', 'maintenance', 'treatment_switch']
...
AGENT_TIERS = {
    ...
    'causal_impact': 'causal_analytics', 'gap_analyzer': 'causal_analytics', 'heterogeneous_optimizer': 'causal_analytics',
    'drift_monitor': 'monitoring', 'experiment_designer': 'monitoring', 'health_score': 'monitoring',
    'prediction_synthesizer': 'ml_predictions', 'resource_optimizer': 'ml_predictions',
    'explainer': 'self_improvement', 'feedback_learner': 'self_improvement'
}
...
class E2IDataGenerator:
    ...
    def _generate_hcp_profiles(self):
        ...
        for i in range(NUM_HCPS):
            ...
            self.hcp_profiles.append({
                ...
                'practice_type': random.choice(['Academic', 'Private', 'Hospital', 'Community']),
                ...
                'affiliation_secondary': [fake.company() if fake else f"Clinic{j}" for j in range(random.randint(0, 2))],
    ...
    def _generate_patient_journeys(self):
        ...
        for i in range(NUM_PATIENTS):
            ...
            journey_start = random_date_in_range(
                self.config.data_start_date, 
                self.config.data_end_date - timedelta(days=30)
            )
            ...
            sources_matched = random.sample(DATA_SOURCES, min(num_sources_matched, len(DATA_SOURCES)))
            ...
            self.patient_journeys.append({
                ...
                'journey_duration_days': (journey_end - journey_start).days if random.random() > 0.3 else None,
                ...
                'primary_diagnosis_desc': random.choice([
                    'Breast Cancer', 'Autoimmune Disorder', 'Psoriasis', 'Rheumatoid Arthritis'
                ]),
                'secondary_diagnosis_codes': [f"ICD10_{random.randint(100, 999)}" for _ in range(random.randint(0, 3))],
                ...
                'comorbidities': random.sample(['Diabetes', 'Hypertension', 'Obesity', 'Heart Disease'], random.randint(0, 3)),
                ...
                'source_combination_method': 'deterministic_match' if len(sources_matched) > 1 else None,
        ...
        for pj in self.patient_journeys:
            ...
            for j in range(num_events):
                ...
                self.treatment_events.append({
                    ...
                    'drug_class': random.choice(['BTK Inhibitor', 'Complement Inhibitor', 'CDK4/6 Inhibitor']),
                    'dosage': f"{random.choice([50, 100, 200, 400])}mg",
                    'duration_days': random.randint(7, 90),
                    'icd_codes': [f"ICD10_{random.randint(100, 999)}" for _ in range(random.randint(1, 3))],
                    'cpt_codes': [f"CPT{random.randint(10000, 99999)}" for _ in range(random.randint(0, 2))],
                    'loinc_codes': [f"LOINC{random.randint(10000, 99999)}" for _ in range(random.randint(0, 2))],
                    'lab_values': {'value': round(random.uniform(0, 100), 2), 'unit': 'mg/dL'} if random.random() > 0.5 else {},
                    ...
                    'outcome_indicator': random.choice(['improved', 'stable', 'declined', None]),
                    'adverse_event_flag': random.random() < 0.05,
                    'discontinuation_flag': random.random() < 0.1,
                    'discontinuation_reason': random.choice(['Side effects', 'Lack of efficacy', 'Cost', None]),
                    'sequence_number': j + 1,
                    'days_from_diagnosis': random.randint(0, 365),
                    'previous_treatment': f"PrevDrug_{random.randint(1, 10)}" if j > 0 else None,
                    'next_treatment': f"NextDrug_{random.randint(1, 10)}" if random.random() > 0.5 else None,
        ...
        for pj in self.patient_journeys:
            ...
            for j in range(num_preds):
                pred_timestamp = datetime.combine(
                    journey_start + timedelta(days=random.randint(1, 60)),
                    datetime.min.time()
                ...
                self.ml_predictions.append({
                    ...
                    'model_type': random.choice(['XGBoost', 'LightGBM', 'CausalForest', 'DoWhy']),
                    ...
                    'feature_importance': random_jsonb(['age', 'comorbidities', 'prior_rx', 'hcp_tier'], 'float'),
                    'shap_values': random_jsonb(['age', 'comorbidities', 'prior_rx', 'hcp_tier'], 'float'),
                    'top_features': [
                        {'name': 'prior_rx', 'importance': round(random.uniform(0.1, 0.3), 4)},
                        {'name': 'comorbidities', 'importance': round(random.uniform(0.05, 0.2), 4)}
                    ],
                    ...
                    'features_available_at_prediction': {
                        ...
                        'feature_names': ['age', 'gender', 'region', 'prior_rx_count', 'days_since_dx']
                    },
    ...
    def _generate_triggers(self):
        ...
        for hcp in self.hcp_profiles:
            ...
            for j in range(num_triggers):
                ...
                change_type = random.choice(['update', 'escalation', 'downgrade']) if is_change else 'new'
                ...
                eligible_patients = [p for p in self.patient_journeys if p['data_split'] == data_split]
                ...
                view_timestamp = delivery_timestamp + timedelta(hours=random.randint(1, 48)) if random.random() > 0.3 else None
                ...
                self.triggers.append({
                    ...
                    'trigger_type': random.choice(['alert', 'recommendation', 'insight', 'next_best_action']),
                    'priority': random.choice(PRIORITY_LEVELS),
                    'confidence_score': round(random.uniform(0.6, 0.99), 3),
                    'lead_time_days': random.randint(7, 60),
                    'expiration_date': (trigger_date.date() + timedelta(days=random.randint(14, 90))).isoformat(),
                    'delivery_channel': random.choice(['email', 'app', 'crm', 'phone']),
                    'delivery_status': random.choice(['pending', 'delivered', 'viewed', 'expired']),
                    'delivery_timestamp': delivery_timestamp.isoformat(),
                    'view_timestamp': view_timestamp.isoformat() if view_timestamp else None,
                    'acceptance_status': random.choice(['accepted', 'rejected', 'pending', None]),
                    'acceptance_timestamp': (view_timestamp + timedelta(hours=random.randint(1, 72))).isoformat() if view_timestamp and random.random() > 0.4 else None,
                    'action_taken': random.choice(['called_patient', 'scheduled_visit', 'sent_info', None]),
                    'action_timestamp': (view_timestamp + timedelta(days=random.randint(1, 7))).isoformat() if view_timestamp and random.random() > 0.5 else None,
                    ...
                    'recommended_action': random.choice(['Schedule follow-up', 'Review patient file', 'Contact for adherence check']),
                    'outcome_tracked': random.random() > 0.3,
                    'outcome_value': round(random.uniform(0.4, 0.9), 3) if random.random() > 0.3 else None,
                    
                    # NEW: Change tracking (WS2 Change-Fail Rate gap)
                    'previous_trigger_id': previous_trigger_id if is_change else None,
                    'change_type': change_type,
                    'change_reason': f"Updated based on {random.choice(['new data', 'model refresh', 'user feedback'])}" if is_change else None,
                    'change_timestamp': trigger_date.isoformat() if is_change else None,
                    'change_failed': change_failed,
                    'change_outcome_delta': round(random.uniform(-0.2, 0.3), 3) if is_change else None,
        ...
        while current_date <= self.config.data_end_date:
            ...
            for agent_name in AGENT_NAMES:
                if random.random() > 0.3:  # Not all agents active every day
                    activity_timestamp = datetime.combine(current_date, datetime.min.time()) + timedelta(
                    ...
                    self.agent_activities.append({
                        ...
                        'activity_type': random.choice(['analysis', 'recommendation', 'action', 'alert', 'experiment']),
                        ...
                        'recommendations': [
                            {'action': f'action_{i}', 'priority': random.choice(PRIORITY_LEVELS)}
    ...
    def _generate_business_metrics(self):
        ...
        metric_names = ['TRx', 'NBRx', 'HCP_Coverage', 'Patient_Touch_Rate', 'Conversion_Rate', 'ROI']
                ...
                for region in REGIONS:
                    for metric_name in metric_names:
                        ...
                        self.business_metrics.append({
                            ...
                            'achievement_rate': round(value / target, 3) if target > 0 else None,
    ...
    def _generate_causal_paths(self):
        ...
        path_nodes = [
            'HCP_Engagement', 'Patient_Adherence', 'Treatment_Response', 
            'Outcome_Improvement', 'Cost_Reduction', 'Market_Share'
        ]
        methods = ['DoWhy', 'EconML', 'CausalML', 'NetworkX', 'Granger']
        
        for i in range(50):
            discovery_date = random_date_in_range(self.config.data_start_date, self.config.data_end_date)
            ...
            self.causal_paths.append({
                ...
                'causal_chain': {
                    'nodes': nodes,
                    'edges': [{'from': nodes[j], 'to': nodes[j+1], 'effect': round(random.uniform(0.1, 0.5), 3)}
                ...
                'confounders_controlled': random.sample(['age', 'region', 'comorbidities', 'prior_treatment'], random.randint(1, 3)),
                'mediators_identified': [nodes[i] for i in range(1, len(nodes)-1)] if len(nodes) > 2 else [],
                'interaction_effects': {
                    'interactions': [{'vars': ['A', 'B'], 'effect': round(random.uniform(-0.1, 0.2), 3)}]
                },
        ...
        while current_date <= self.config.data_end_date:
            for source in DATA_SOURCES:
                ...
                self.data_source_tracking.append({
                    ...
                    'source_name': source,
                    'source_type': 'claims' if 'IQVIA' in source else random.choice(['lab', 'emr', 'crm']),
                    
                    'records_received': records_received,
                    'records_matched': records_matched,
                    'records_unique': records_received - random.randint(100, 1000),
                    
                    'match_rate_vs_iqvia': round(random.uniform(0.8, 0.95), 3) if source != 'IQVIA_APLD' else 1.0,
                    'match_rate_vs_healthverity': round(random.uniform(0.75, 0.92), 3) if source != 'HealthVerity' else 1.0,
                    'match_rate_vs_komodo': round(random.uniform(0.78, 0.93), 3) if source != 'Komodo' else 1.0,
                    'match_rate_vs_veeva': round(random.uniform(0.7, 0.88), 3) if source != 'Veeva' else 1.0,
    ...
    def _generate_ml_annotations(self):
        ...
        annotation_types = ['diagnosis_validation', 'outcome_label', 'treatment_response', 'adverse_event']
        ...
        for pj in self.patient_journeys[:50]:  # Annotate subset
            ...
            for annotator_idx in range(num_annotators):
                ...
                self.ml_annotations.append({
                    ...
                    'is_adjudicated': annotator_idx == num_annotators - 1 and random.random() > 0.3,
                    'adjudication_result': {'final_label': random.choice(['positive', 'negative'])} if annotator_idx == num_annotators - 1 else None,
                    'adjudicated_by': f"ADJ_{random.randint(1, 5):03d}" if annotator_idx == num_annotators - 1 else None,
                    'adjudicated_at': datetime.now().isoformat() if annotator_idx == num_annotators - 1 else None,
    ...
    def _generate_etl_pipeline_metrics(self):
        ...
        pipeline_names = ['patient_journey_etl', 'treatment_event_etl', 'hcp_profile_etl', 'claims_integration']
        ...
        while current_date <= self.config.data_end_date:
            for pipeline_name in pipeline_names:
                ...
                source_data_timestamp = run_start - timedelta(hours=random.randint(12, 168))  # 0.5 to 7 days
                ...
                self.etl_pipeline_metrics.append({
                    ...
                    'source_data_date': (current_date - timedelta(days=random.randint(1, 7))).isoformat(),
                    ...
                    'error_details': None if random.random() > 0.1 else 'Timeout on large batch',
            ...
            for survey_idx in range(num_surveys):
                ...
                for brand in TARGET_BRANDS:
                    ...
                    self.hcp_intent_surveys.append({
                        ...
                        'days_since_last_survey': random.randint(30, 90) if previous_survey_id else None,
                        
                        'interventions_since_last': [
                            {'type': random.choice(['detail', 'email', 'webinar', 'sample']), 'date': (survey_date - timedelta(days=random.randint(1, 30))).isoformat()}
    ...
    def _generate_preprocessing_metadata(self):
        ...
        self.preprocessing_metadata = {
            ...
            'feature_distributions': {
                f: {'mean': round(random.uniform(0.3, 0.7), 4), 'std': round(random.uniform(0.1, 0.3), 4)}
    ...
    def _print_summary(self):
        ...
        print(f"  - Data Source Tracking:{len(self.data_source_tracking):,} (WS1 Cross-source/Stacking)")
        ...
        print(f"  - HCP Intent Surveys:  {len(self.hcp_intent_surveys):,} (Brand Intent-to-Prescribe)")
    ...
    def export_to_json(self, output_dir: str = "."):
        ...
        validation_data = {
            'patient_journeys': [p for p in self.patient_journeys if p['data_split'] == 'validation'],
            'treatment_events': [e for e in self.treatment_events if e['data_split'] == 'validation'],
            ...
            'agent_activities': [a for a in self.agent_activities if a['data_split'] == 'validation'],
            'business_metrics': [m for m in self.business_metrics if m['data_split'] == 'validation'],
