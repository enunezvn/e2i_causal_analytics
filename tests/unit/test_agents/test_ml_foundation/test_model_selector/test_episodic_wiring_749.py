"""#749: model_selector must record its run to EPISODIC memory. Its store_model_selection hook was defined but
...
@pytest.mark.unit
def test_update_episodic_memory_invokes_store_model_selection_with_uuid_session():
    ...
    with patch("src.agents.ml_foundation.model_selector.agent.ModelSelectorMemoryHooks") as HookCls:
        ...
        hook.store_model_selection = AsyncMock(return_value="mem-1")
        asyncio.run(agent._update_episodic_memory(_DATA))

    hook.store_model_selection.assert_awaited_once()
    kwargs = hook.store_model_selection.await_args.kwargs
