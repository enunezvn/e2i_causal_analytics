...
| **Experiment Monitor** | Standard (Fast) | Live A/B experiment health monitoring | Chi-squared SRM, threshold checks, no LLM |
...
class HealthScoreOutput(BaseModel):
    ...
    # Per-dimension scores are Optional and FAIL-CLOSED: an unmeasured dimension
    # (no real backend wired, a skipped scope, or a failed check) is None — NEVER
    # a fabricated healthy 1.0 — and is EXCLUDED from the composite rather than
    # counted as healthy. Only genuinely measured dimensions contribute.

    # Component health
    component_statuses: List[ComponentStatus] = Field(..., description="Status of system components")
    component_health_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Component health (0-1); None if unmeasured")

    # Model health
    model_metrics: List[ModelMetrics] = Field(..., description="Model performance metrics")
    model_health_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Model health (0-1); None if unmeasured")

    # Pipeline health
    pipeline_statuses: List[PipelineStatus] = Field(..., description="Data pipeline statuses")
    pipeline_health_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Pipeline health (0-1); None if unmeasured")

    # Agent health
    agent_statuses: List[AgentStatus] = Field(..., description="Agent availability")
    agent_health_score: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Agent health (0-1); None if unmeasured")

    # Composite score
    overall_health_score: float = Field(..., ge=0.0, le=100.0, description="Overall health (0-100), composed only from MEASURED dimensions")
    health_grade: HealthGrade = Field(..., description="Letter grade (A-F)")

    # Provenance of the composite (fail-closed): "measured" (all 4 dims measured),
    # "partial" (1-3), or "unknown" (0). Defaults to "unknown" so any path that
    # forgets to set it fails closed, not open.
    data_provenance: str = Field(default="unknown", description="measured | partial | unknown")
...
> **Cross-reference — `resource_optimizer`**: the `resource_optimizer` entries in the handoff example above are illustrative only (a sample agent surfaced by the health rollup as a low-success-rate example). `resource_optimizer` is a **Tier 4** agent; its authoritative input/output contract lives in `tier4-contracts.md` §3 (Resource Optimizer Agent), not here. Tier 3 references it only as an example subject of a health check.

---

## 5. Experiment Monitor Agent

**Agent Type**: Standard (Fast Path)
**Primary Methods**: Deterministic checks — chi-squared SRM, enrollment/freshness thresholds, interim-analysis triggers, Digital-Twin fidelity (no LLM)
**Latency**: Up to 15s (orchestrator SLA); <5s per experiment-check target

The Experiment Monitor watches **already-running** A/B experiments for operational health problems: Sample Ratio Mismatch (SRM), enrollment shortfalls, stale data, interim-analysis triggers, and Digital-Twin prediction fidelity. It is a deterministic, no-LLM agent — every finding comes from a statistical test or a threshold check, never a language-model judgment.

**Boundary**: this agent covers experiments that are *live*. Designing a *new* experiment — sample-size / power analysis, randomization strategy, validity audit, pre-registration — is owned by the **Experiment Designer** (§2). The intent classifier ranks `experiment_monitor` as the most-specific experiment intent, ahead of `experiment_design`, so "check / monitor my running experiment" resolves here while "design an experiment" resolves to the designer.

> **Note**: unlike the other Tier 3 agents, the Experiment Monitor's I/O contract is expressed with Python **dataclasses** (not Pydantic `BaseModel`), and it exposes **no** `get_handoff` — hence no §5.x Handoff Format below.
...
class ExperimentMonitorInput:
    """Input contract for Experiment Monitor Agent."""

    query: str = ""
    experiment_ids: Optional[List[str]] = None   # specific experiments; None + check_all_active sweeps all
    check_all_active: bool = True

    # Detection thresholds
    srm_threshold: float = 0.001                 # SRM chi-squared p-value threshold
    enrollment_threshold: float = 5.0            # minimum daily enrollment rate
    fidelity_threshold: float = 0.2              # maximum acceptable Digital-Twin prediction error
    stale_data_threshold_hours: float = 24.0

    # Selective-check toggles (each check can be disabled independently)
    check_interim: bool = True
    check_srm: bool = True
    check_enrollment: bool = True
    check_fidelity: bool = True

    # Provenance opt-in (#894): include synthetic-tagged rows (default excludes them)
    include_synthetic: bool = False
    brand: Optional[str] = None                  # restrict the sweep to one brand_type; None = all brands
```
...
class ExperimentMonitorOutput:
    """Output contract for Experiment Monitor Agent."""

    experiments: List[ExperimentSummary] = field(default_factory=list)
    alerts: List[MonitorAlert] = field(default_factory=list)
    experiments_checked: int = 0     # capped at 25 per sweep
    total_running: int = 0           # honest portfolio size matching the sweep scope (not capped)
    healthy_count: int = 0
    warning_count: int = 0
...
Alert and per-experiment records are `TypedDict`s in `state.py`: `MonitorAlert` (alert_type ∈ `srm | enrollment | stale_data | fidelity | interim_trigger`, with severity `info | warning | critical`) and `ExperimentSummary` (health_status ∈ `healthy | warning | critical | unknown`, with a `health_reason` explaining the flag). Per-check issue types include `SRMIssue`, `EnrollmentIssue`, `StaleDataIssue`, `FidelityIssue`, and `InterimTrigger`.

### 5.3 Internal Pipeline

Sequential LangGraph workflow, deterministic (no LLM), executed under **one** SINGLE_AGENT dispatch:

```
audit_init → health_checker → srm_detector → interim_analyzer → fidelity_checker → alert_generator → END
```

- **audit_init** — genesis block of the tamper-evident audit chain; each business node below is wrapped so it emits a timed audit entry (`duration_ms`).
- **health_checker** — experiment health, enrollment-rate shortfalls, and stale-data freshness.
- **srm_detector** — Sample Ratio Mismatch via chi-squared test against the expected allocation ratio.
- **interim_analyzer** — interim-analysis triggers / information-fraction milestones.
- **fidelity_checker** — Digital-Twin prediction fidelity (simulated vs actual effect).
- **alert_generator** — aggregates issues into `MonitorAlert` records with severity + recommended actions.

Five business nodes plus the `audit_init` genesis. (Note: `src/agents/experiment_monitor/CONTRACT_VALIDATION.md` §1.1 still describes "4 nodes"; the compiled graph wires five business nodes — the implementation is authoritative.)

### 5.4 Dispatch & Routing

- **Intent key**: `experiment_monitor` → `experiment_monitor` agent (`src/agents/orchestrator/nodes/router.py:149-158`).
- **Priority**: `critical`; **timeout**: 15000 ms; **fallback_agent**: `None`.
- **Not discovery-aware** — absent from `DISCOVERY_AWARE_AGENTS`, so it never receives a discovered causal DAG in its parameters.
- The classifier ranks `experiment_monitor` as the most-specific experiment intent (above `experiment_design`), so monitoring a live experiment routes here rather than to the Experiment Designer.

---

## 6. Inter-Agent Communication

### 6.1 Orchestrator → Tier 3 Dispatch

All Tier 3 agents receive dispatches via the standard `AgentDispatchRequest` (see `orchestrator-contracts.md`).

### 6.2 Tier 3 → Orchestrator Response

All Tier 3 agents return via the standard `AgentDispatchResponse`.

### 6.3 Tier 3 Inter-Agent Handoffs
...
### 8.1 Experiment Designer Error Patterns
...
### 8.2 Drift Monitor Error Patterns
...
### 8.3 Health Score Error Patterns
...
|---------|------|---------|--------|
| 1.2 | 2026-07-29 | Doc-rot fix (#1344): added §5 Experiment Monitor Agent (contract had none despite CONTRACT_VALIDATION.md claiming tier3 alignment); documented Health Score's Optional / fail-closed per-dimension scores; disambiguated the `resource_optimizer` cross-mention to its Tier 4 home | Claude |
...
### 13.1 Sender Agents (Drift Monitor, Experiment Designer)
...
### 13.2 Recipient Agent (Health Score)
...
### 13.3 Signal Quality Thresholds
