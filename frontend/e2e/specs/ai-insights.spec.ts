...
test.describe('AI Agent Insights Page', () => {
  ...
  test.describe('Active Causal Chains', () => {
    ...
    test('should have zoom controls', async ({ page }) => {
      // Scope to the CardHeader that contains the "Active Causal Chains"
      // title. We climb from the title element to its nearest CardHeader
      // ancestor instead of using `locator('div').filter()`, which would
      // resolve to whichever broad ancestor matched first and could pass
      // even when the graph controls are missing.
      const title = page.getByText(/Active Causal Chains/i).first();
      const cardHeader = title.locator(
        'xpath=ancestor::*[contains(@class, "CardHeader") or starts-with(@class, "pb-3") or contains(@class, "flex-row")][1]',
      );
      // Fall back to the title's nearest sibling-bearing ancestor if class
      // names get tree-shaken in prod build.
      const zoomButtons = (
        (await cardHeader.count()) > 0 ? cardHeader : title.locator('xpath=ancestor::div[1]')
