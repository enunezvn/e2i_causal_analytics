...
| Ontology | `config/ontology/*.yaml` | YAML (17 files) — design spec; no runtime loader (`src/ontology`'s schema compiler, validator and inference engine run in CI tests only; its `vocabulary_registry` / `query_extractor` serve `config/domain_vocabulary.yaml` at runtime). The graph is built by code: see `docs/data/04-KNOWLEDGE-GRAPH-ONTOLOGY.md` "How the graph is built". CI (`test_kg_writer_ontology_drift.py`) fails when code writes a node label or edge type the ontology does not declare |
