...
- `fix(chat)` after codex iteration 2 (three findings, all reproduced) and the final whole-branch review: `degraded` keeps naming the fields whose refresh failed even when last-good fields are carried forward (60 s retry, visible outage) and the renderer keys its fallbacks on empty data; the extends-list forbids explaining WHY rather than any `explain*`; `individual_prediction` yields to aggregate HCP-segment likelihood asks (`_AGGREGATE_LIKELIHOOD_RE` / `_INDIVIDUAL_ASK_RE`); NEVER list names model retraining and per-HCP / per-patient predictions; a review then closed the exemption for ranked HCP-list asks carrying an axis word (`_INDIVIDUAL_ASK_RE` names top-N / listed HCPs) and narrowed "drivers of" to "drivers behind". Catalog tests 85 -> 96.
- `fix(chat)` after codex iteration 3 (two over-blocks): the aggregate likelihood exemption accepts "HCP specialty/specialties"; the competitor rule's "versus competitors" alternative needs a share / volume / performance word so competitor-landscape context asks (section E) are kept; a review then made the rule bidirectional with the platform's KPI words and the noun "competition". Catalog tests 96 -> 110.
- `fix(copilotkit,chat)` after codex iteration 4 (three findings): the readables note carries the prose-summary wording only when a prose readable is present (`_is_json_text`), so pages without a summary keep their previous prompt byte-identical; the shap rule's "top N features" alternative excludes platform / dashboard product questions; the competitor data words include starts, initiations, switches, units, demand. Catalog tests 110 -> 116; readable-note tests 2 -> 4.
- codex iteration 5: ACCEPT with 0 findings. A reviewed robustness commit followed: `_is_json_text` runs on the truncated readable value and treats RecursionError as not-JSON (a 100k-deep nested value renders truncated instead of raising); the shap rule's "top features" lookahead accepts a bare "E2I" (but not "the E2I model"); a second review moved the JSON check back onto the full value so an oversized JSON readable keeps the JSON-only wording. Catalog tests 116 -> 118; readable-note tests 4 -> 6.
